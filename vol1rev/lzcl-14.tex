...
物理学家们在幼儿园里就学会了处理这类问题的方法: 求解有外场的\,Dirac\,波动方程. 在这里推导这一方程看起来似乎是不必要的, 但正如第1章中所强调的, 
Dirac\,将这一方程作为一种相对论\,Schr\"{o}dinger\,方程的原始动机并没有经受住检验.
...
在结束本节之前, 对一般的静电势$\mathscr{A}^{0}$, 我们将考察波函数和矩阵元在非相对论情况下的近似形式. (对\,Coulomb\,势, %
...
$S_{\mathscr{A}}^{\prime}=S_{\mathscr{A}}+\delta S_{\mathscr{A}}$, 其中修正项为
\begin{equation}
\delta S_{\mathscr{A}}(x,y)=\int \dif^{4}z\int \dif^{4}w\:S_{\mathscr{A}}(x,z)\,\Sigma
...
用能量变量代替时间变量, 这是
\begin{equation}
\delta S_{\mathscr{A}}(\bx,\by;E)=\int \dif^{3}z\int \dif^{3}w\:S_{%
...
这些辐射修正的效果是将波函数变成$U_{N}=u_{N}+\delta u_{N}$和$V_{N}=v_{N}+\delta v_{N}$, %
并将束缚态能量变成$E_{N}^{\prime }=E_{N}+\delta E_{N}$, 所以全传播子是
\begin{align}
S_{\mathscr{A}}^{\prime }(\bx,\by;E) &\simeq S_{\mathscr{A}}(%
\bx,\by;E)  \nonumber \\
&\quad+\sum_{N}\frac{\delta u_{N}(\bx)\bar{u}_{N}(\by)+u_{N}(%
\bx)\delta \bar{u}_{N}(\by)}{E_{N}-E}  \nonumber \\
&\quad-\sum_{N}\frac{\delta v_{N}(\bx)\bar{v}_{N}(\by)+v_{N}(%
\bx)\delta \bar{v}_{N}(\by)}{E_{N}+E}  \nonumber \\
&\quad-\sum_{N}\frac{u_{N}(\bx)\bar{u}_{N}(\by)\delta E_{N}}{%
(E_{N}-E)^{2}}+\sum_{N}\frac{v_{N}(\bx)\bar{v}_{N}(\by)\delta
E_{N}}{(E_{N}+E)^{2}}\:.  \label{14.2.14}
\end{align}%
(因为我们现在没有在散射态的连续谱中取$E$\:, 所以我们扔掉了$\mi\epsilon$项.) %
我们看到电子束缚态的能量位移$\delta E_{N}$由全传播子中$-u_{N}(\bx)\bar{u}%
...
\begin{align}
\delta S_{\mathscr{A}}(\bx,\by;E) &=\sum_{N,M}\frac{u_{N}(%
...
&-\mi(Z_{2}-1)(\gamma ^{\mu }\partial _{\mu }+m)\updelta ^{4}(x-y)+\mi\delta m\,\updelta ^{4}(x-y)  \nonumber \\
...
\begin{equation}
\delta E_{N}=-\int \dif^{3}p^{\prime }\int \dif^{3}p\:\bar{u}_{N}(\bp^{\prime
...
我们现在来考虑一般静电场中非相对论性电子能级的辐射修正, 例如处在$Z\alpha \ll 1$的轻核的\,Coulomb\,场中的电子. %
...
\end{equation}%
我们将方程(\ref{14.2.24})前两项(包括抵消项$Z_{2}-1$和$Z_{2}\delta m$的式子)中的光子传播子写成如下形式
\begin{equation}
...
\subsection*{\textit{A} \  高\quad 能\quad 项}
...
程(\ref{14.2.24})和(\ref{14.2.23})给出. 外场的零阶项简单地抵消掉了: $\delta m$项抵消了$\mathscr{A}=0$的第一项; %
...
\begin{align}
\lbrack \delta E_{N}]_{\mbox{\scriptsize 高能}} &=\mi e\int \dif^{3}p^{\prime }\int
...
\begin{align}
&[ \delta E_{N}]_{F_{1}} =-\frac{e^{2}}{24\uppi ^{2}m_{e}^{2}}\left[
...
\lbrack \delta E_{N}]_{F_{1}} &=-\frac{e^{2}}{24\uppi ^{2}m_{e}^{2}}\left[
\ln \left( \frac{\mu ^{2}}{m_{e}^{2}}\right) +\frac{2}{5}+\frac{3}{4}\right]
\nonumber \\
...
而在位置空间的结果是%
\begin{equation}
[ \delta E_{N}]_{F_{1}}=\frac{e^{2}}{24\uppi ^{2}m_{e}^{2}}\left[ \ln
...
\begin{equation}
[\delta E_{jnl}]_{F_{1}}=-\frac{2Z^{4}\alpha ^{5}m_{e}}{3\uppi n^{3}}%
\left[ \ln \left( \frac{\mu ^{2}}{m_{e}^{2}}\right) +\frac{2}{5}+\frac{3}{4}%
\right] \updelta _{\ell ,0}\:.  \label{14.3.14}
\end{equation}%
($\delta E$不依赖总角动量$z$-分量$m$是由旋转不变性保证的.) 方程(\ref{14.3.12})和(\ref{14.3.13})括号中的$%
...
\begin{equation}
\delta E_{N}=-\frac{2Z^{4}\alpha ^{5}m_{e}}{3\uppi n^{3}}\left[ \ln \Bigl(
Z^{4}\alpha ^{4}\Bigr) \delta _{\ell ,0}+O(1)\right] \:. \label{14.3.15}
\end{equation}%
对氢原子的$2s$态, 对数项单独给出
\[
\delta E_{2s}\simeq -\frac{\alpha ^{5}m_{e}}{12\uppi }\ln \Bigl( \alpha
...
\begin{align}
[\delta E_{N}]_{F_{2}} &=-\frac{e^{2}}{32\uppi ^{2}m_{e}}\int
...
或者, 在位置空间,
\begin{equation}
\lbrack \delta E_{N}]_{F_{2}}=\frac{\mi e^{2}}{64\uppi ^{2}m_{e}}\int \dif^{3}x\:(\bar{%
...
对$\hAAA=0$的纯静电场, 这是
\begin{equation}
[\delta E_{N}]_{F_{2}}=\frac{-\mi e^{2}}{32\uppi ^{2}m_{e}}\int
...
\begin{align}
\lbrack \delta E_{N}]_{F_{2}} &=\frac{e^{2}}{32\uppi ^{2}m_{e}^{2}}\int
...
\begin{align}
\lbrack \delta E_{N}]_{\mbox{\scriptsize 高能}} &=\frac{e^{2}}{24\uppi
...
\subsection*{\textit{B} \  低\quad 能\quad 项}
...
取代采用动量空间的公式(\ref{14.2.24}), 回到构形空间公式(\ref{14.2.18})会更方便一些. 这给出电子自能函数的低能贡献
\begin{align}
[\Sigma _{\mathscr{A}}^{\ast }(x,y)]_{\mbox{\scriptsize 低能}}
...
\begin{align}
\lbrack \Sigma _{\mathscr{A}}^{\ast }(\bx,\by;E)]_{\mbox{\scriptsize 低能}} &=\frac{\mi e^{2}}{(2\uppi )^{4}}\int \dif^{4}k\:\gamma ^{\rho }\,S_{%
...
\begin{align}
&[\delta E_{N}]_{\mbox{\scriptsize 低能}} =-\int \dif^{3}x\int \dif^{3}y\:\bar{u}%
_{N}(\bx)\,[\Sigma _{\mathscr{A}}^{\ast }(\bx,\by%
;E_{N})]_{\mbox{\scriptsize 低能}}\,u_{N}(\by)  \nonumber \\
...
{14.3.27})现在变成
\begin{eqnarray}
&&\lbrack \delta E_{N}]_{\mbox{\scriptsize 低能}} = -\frac{e^{2}}{2(2\uppi )^{3}}\int
\dif^{3}k \,\sum_{M}  \nonumber \\
...
\begin{eqnarray}
&&\lbrack \delta E_{N}]_{\mbox{\scriptsize 低能}} =  \nonumber \\
...
\begin{align}
\lbrack \delta E_{N}]_{\mbox{\scriptsize 低能}} &=\frac{e^{2}}{2(2\uppi )^{3}}%
...
\begin{equation}
[ \delta E_{N}]_{\mbox{\scriptsize 低能}}=\frac{e^{2}}{6\uppi ^{2}}%
...
项抵消了:%
\begin{align}
\delta E_{N} &=[\delta E_{N}]_{\mbox{\scriptsize 高能}}+[\delta E_{N}]_{\mbox{\scriptsize 低能}}  \nonumber \\
...
5. 对习题\,4\,计算出$m_{\phi}=0$的结果.
 ...
 \end{KAI}
  \markboth{第14章\quad 外场中的束缚态}{参~\,考~\,文~\,献}      %%前双后单书眉
