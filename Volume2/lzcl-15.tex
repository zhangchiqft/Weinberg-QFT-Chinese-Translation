...
\pagenumbering{arabic}
 ...
 \]%
 结构常数现在变成了实的并且是全反对称的: $C^{a}{}_{bc}=\epsilon _{abc}$. 这时$g_{ab}=2\updelta_{ab}$, 并且代数是紧致的. 
 ...
 的二次型, 其中二阶项$\Pi _{\alpha i}(x)\Pi _{\beta j}(y)$的系数由不依赖场的核$\frac{1}{2}\updelta^{4}(x-y)\updelta_{ij}$给出. 假定这对物质场$\pi _{\ell }$同样是成立的, 通过取$\pi_{\ell }$和$\Pi _{\alpha i}$等于作用量(\ref{15.4.1})的驻定``点'', 可以算出对$\pi
 ...
 }\partial y^{\nu }}\,\updelta^{4}(x-y)\updelta_{\alpha \beta }+\epsilon \text{%
 项} \\
 ...
 或者以(\ref{15.7.3})的形式%
 \begin{equation}
 \updelta_{\theta }f_{\alpha }[x;A,\psi ]=\theta \Delta _{\alpha }(x;A,\psi
 ,\omega )\:.   \label{15.7.23}
 \end{equation}%
 ...
 \rangle $在$\Psi $的变化$\tilde{\updelta}\Psi$时的变化是
 \begin{equation}
 ...
 以方程(\ref{15.8.1})的记法, 在变换(\ref{15.1.9})中 变分$\updelta_{A}\phi ^{r}$为
 \[
 ...
 而$B[f]$是$f_{A}$的泛函, 它的选择多少有些任意(例如$\prod_{A}\updelta(f_{A})$.) 常数$\Omega$是规范群的体积元, 而常数$C$(像方程(\ref{15.5.19})那样)的定义为
 \begin{equation}
 ...
 \end{equation}%
 在方程(\ref{15.8.9})中, 下标$L$代表左微分, 它的定义满足如果$\updelta F=\updelta \chi \:G$, 那么$\updelta_{L}F/\updelta \chi =G$, 
 ...
 \end{equation}%
 其中$\updelta F$是$\chi $和$\chi ^{\ddag }$的无限小泛函, 除了为使$S^{\prime}$是玻色的且鬼数为零而要求$\updelta F$必须是费米的且鬼数为$-1$以外, 
 它是任意的. 特别地, 取$\updelta F$仅为$\chi^{n}$的费米泛函$\epsilon \Psi $就给出
 \begin{equation}
 ...
 \label{15.9.25}
 \end{equation}%
 例如, 考察由无限小费米算符$\updelta F$生成的无限小反正则变换, 在该变换下, 所有玻色泛函或费米泛函$G$被变换为
 \begin{equation}
 ...
 米的, 由于对于这些项, $%
 \updelta _{L}^{2}\Psi /\updelta \chi ^{m}\updelta \chi ^{n}$是反对称的, 所以该项是反对称的.}的, 所以也为零.
 ...
 \right) _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi }\left( \frac{\delta
 ...
 \begin{equation}
 (S,S) - 2 \mi \Delta S = 0 \qquad \quad \text{在} \quad \chi^{\ddag}=\updelta \Psi/\delta\chi \:.  \label{15.9.35}
 \end{equation}
 ...
 假定量子主方程(\ref{15.9.35})是满足的, $\Psi $变化$\updelta \Psi $引起的算符$\mathcal{O}[\chi ]$的真空期望值的变化是\begin{equation}
 ...
 下不变的算符,\footnote{对于开规范理论, 一般而言, 除了常数以外, 没有在变换(\ref{15.9.16})---(\ref{15.9.17})下不变的算符.  取而代之, 应该考虑的是在幂零的``量子BRST算符''$\sigma$下不变的算符$O(\chi,\chi^{\ddag})$, 其中$\sigma$定义为$\sigma O = (O,S)-\mi \Delta O$. 对于仅依赖于$\chi$的$O$, 这一条件退化为方程(\ref{15.9.36}). 如果$\sigma O=0$, 那么规范固定费米子$\Psi$的微小变换不会影响$O(\Psi, \updelta\Psi/\updelta \chi)$的期望值.\cite{24}} 它们的真空期望值不被规范固定费米子$\Psi $的变化所影响. 两个或多个算符的真空期望值的相应结果同样成立.
