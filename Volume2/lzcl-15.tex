...
在电动力学中, 电荷为$e_{n}$的场$\psi _{n}(x)$进行了参数为任意函数$\Lambda(x)$的规范变换
...
用它来构造规范协变导数$\partial_{\mu}\psi_{n}(x)-\mi e_{n}A_{\mu}(x)\psi_{n}(x)$, 
...
理论中的大耦合常数似乎完全妨碍了使用微扰论.
 ...
 最后发现, {\kai{所有}}观测到的基本粒子相互作用都是由伴随定域规范对称性的矢量粒子生成的; 
 相应的自旋1粒子要么由于规范对称性自发破缺而获得非常大的质量, 要么由于耦合常数在远距离处变大而被``禁闭''了. 
 ...
 \begin{equation}
 \updelta \psi _{\ell }(x)=\mi\,\epsilon ^{\alpha }(x)(t_{\alpha })_{\ell }{}^{m}\psi _{m}(x)  \label{15.1.1}
 \end{equation}%
 ...
 \end{equation}%
 任何一组满足方程(\ref{15.1.3})和(\ref{15.1.5})的常数${C^{\gamma}}_{\alpha\beta}$至少定义了一组矩阵${t^{\text{A}}}_{\alpha}$:%
 \begin{equation}
 ...
 $\epsilon _{\gamma \alpha \beta }$分别是$+1$或$-1$, 其他情况下则为零. 
 我们发现这与3维旋转群的Lie代数(2.4.18)相同; 这里的矩阵构成了我们所说的该Lie代数的自旋1/2表示. 
 伴随表示的矩阵(\ref{15.1.6})在这里是(在行和列标记为1,2,3的基下):%
 \[
 t^{\text{A}}{}_{1}=\begin{bmatrix}
 0 & 0 & 0 \\
 0 & 0 & -\mi \\
 0 & \mi & 0%
 \end{bmatrix}  \: ,\qquad 
 t^{\text{A}}{}_{2}=\begin{bmatrix}
     0 & 0 & \mi \\
     0 & 0 & 0 \\
     -\mi & 0 & 0% 
 \end{bmatrix} \:, \qquad 
 ...
 这个任务是很简单的——物质场的任何函数只要在$\epsilon ^{\alpha }$为常数的变换(\ref{15.1.1})下不变, 
 那么在$\epsilon ^{\alpha }$为时空坐标的任意实函数时, 它在变换(\ref{15.1.1})下也将是不变的. 
 如果拉格朗日量包含场的导数项(也是它所必须的)就不会是这种情况, 这是因为有了依赖位置的函数$\epsilon ^{\alpha }(x)$,
 物质场的导数就不会像场本身那样变换. 对方程(\ref{15.1.1})两边做微分给出
 \begin{equation}
 \updelta \Big( \partial _{\mu }\psi _{\ell }(x)\Big) =\mi\,\epsilon ^{\alpha
 ...
 ^{\alpha }$, 用来抵消方程(\ref{15.1.8})中的第二项. 既然这个场携带了一个$\alpha $指标, 我们希望它%
 也经历一个类似方程(\ref{15.1.1})的矩阵变换, 所不同的只是把$t_{\alpha }$替换成伴随矩阵表示(\ref{15.1.6}). 因此, 
 我们先尝试着把新``规范'' 场的变换关系取为
 \[
 \updelta A^{\beta}{}_{\mu }=\partial _{\mu }\epsilon ^{\beta}+
 ...
 \updelta (D_{\mu }\psi )_{\ell } &=\mi\,\epsilon ^{\alpha }\,(t_{\alpha })_{\ell }^{%
 ...
 \end{equation}%
 在$\Lambda ^{\alpha }(x)$是一个无限小的$\epsilon ^{\alpha }(x)$的极限下, 方程(\ref{15.1.15})和(\ref{15.1.17})退化成前面的变换规则(\ref{15.1.1})和(\ref{15.1.9}).
 
 我们从方程(\ref{15.1.17})中可以看到, 通过合适地选择$\Lambda ^{\beta }(x)$, 总可以使$%
 {A^{\alpha}}_{\mu \Lambda}(x)$在任何{\kai{一个}}点处为零, 记该点$x=z$. (简单地令$\Lambda ^{\alpha }(z)$为零, 并在$x=z$处令$\partial \Lambda ^{\alpha }(x)/\partial x^{\mu}=-{A^{\alpha}}_{\mu }(x)$.) 另外, 至少在任意一个给定点邻近的有限区域内, 
 总可以对$\Lambda ^{\beta }(x)$进行选择, 使${A^{\alpha}}_{\mu \Lambda}(x)$的任意{\kai{一个}}时空分量对所有$\alpha $为零. 例如, 
 ...
 我们现在将要集中考察拉格朗日量中只依赖$F^{\alpha }{}_{\mu \nu }$的项. 和电动力学中一样, 对任何自旋一的无质量粒子, 
 拉格朗日量必须包含一个$\partial _{\mu }A^{\alpha}{}_{\nu }-\partial _{\nu }A^{\alpha }{}_{\mu }$的二次项作为自由粒子项, 
 ...
 $的动能项, 场强$F^{\alpha}{}_{\mu \nu}$的定义(\ref{15.1.13})中的二次部分给出了方程(\ref{15.2.3})中的相互作用项. 
 这是非阿贝尔规范理论类似于广义相对论的又一个方面, 在广义相对论中,
 引力场拉格朗日量的动能部分被包含在Einstein-Hilbert拉格朗日密度${-}\sqrt{%
 g}R/8\uppi G$中, 这一项也包含了引力场的自作用. 两种情况的原因是相似的:
 凡是携带能量和动量的, 引力场都会与之相互作用, 所以引力场与自身相互作用, 而凡是按照规范群的非平庸表示进行变换的, 规范场都会与之相互作用, 所以规范场与自身相互作用.
%  引力场与自身相互作用是因为它与任何携带能量和动量的东西相互作用, 
%  而规范场与自身相互作用是因为它与任何按照规范群的非平庸表示(在伴随表示下)变换的物质相互作用. 
这与电动力学的情况相反, 在电动力学中, 光子不携带与之相互作用的量子数——电荷, 因而对电磁场有可能引入不含相互作用的动能项$-\frac{1}{4}F_{\mu \nu}F^{\mu \nu }$.
 
 数值矩阵$g_{\alpha \beta }$可以取成对称的, 并且必须取成实的以给出实的拉格朗日量. 为使这一项满足规范不变性要求(\ref{15.2.2}), 
 对所有的$\delta $, 我们必须有:%
 \[
 ...
 \mathscr{S}_{\alpha \beta }t_{\beta }$, 其中$\mathscr{S}$是非奇异实矩阵), 使得结构常数$\tilde{C}^{\alpha }{}_{\beta\gamma}$不%
 ...
 \[
 C^{\ell c}{}_{na}{}_{mb}=\updelta_{\ell m}\updelta_{mn}C^{(n)c}{}_{ab}\:, 
 \]%
 ...
 \]%
 方程(\ref{15.A.10})所给出的度规在这里是对角的, 对角元为:%
 \[
 g_{11}=g_{22}=-g_{33}=-2\:. 
 \]%
 这不是正定矩阵, 所以Lie代数不是紧致的. 事实上它是非紧群$O(2,1)$的Lie代数, 该群是两维空间一维时间中的Lorentz群.
 
 若两组生成元相差的是一个非奇异的{\kai{实}}线性变换, 那么就认为这两组生成元张开的是同一个Lie代数, 并且生成的是同一个群. 
 ...
 \]%
 结构常数现在是实的并且是全反对称的: $C^{a}{}_{bc}=\epsilon _{abc}$. 这时$g_{ab}=2\updelta_{ab}$, 并且代数是紧致的. 
 当然, 我们认出了这是熟悉的三维旋转的紧致群$O(3)$的代数. 为了看到这对任何单Lie代数总是可能的, 
 只需注意到方程(\ref{15.A.10})所定义的矩阵$g_{a b}$是实的, 对称的且非奇异的, 
 从而可以通过一个实正交变换将其变成主对角线上才有非零元的对角形式. 然后, 给这一基下与$g_{ab}$的负对角元对应的所有生成元乘以因子$\mi$即可.
 
 我们不加证明地指出, 紧Lie群的有限维表示都是幺正的, 相应地, 紧Lie代数的有限维表示都是厄米的. 进一步, 容易看到, 
 如果Lie代数想要有任何基于独立的有限维厄米矩阵$t_{\alpha }$的非平庸表示, 它只能是$U(1)$与紧致单Lie代数的直和. 为了证明这一点, 
 我们可以简单地定义
 \[
 ...
 它满足不变性条件(\ref{15.2.4}), 并且在本章的附录A中我们证明了这个结果等价于Lie代数上的一个条件: 该Lie代数是紧致单纯子代数与$U(1)$子代数的直和. 对我们的目的来说, 这一结果重要之处在于, 单Lie代数都是某些限定类型和维数的代数. 例如, 很容易看到不存在生成元少于3个的单Lie代数, 
 这是因为在一维或二维中, 不存在3个指标全反对称的结构常数. 有了三个生成元, 通过将$%
 C^{3}{}_{12}$, $C^{2}{}_{31}$和$C^{1}{}_{23}$都取成非零的,
 可以避免不变子代数. 在结构常数都为实数且全反对称的基下, 显然只有一种可能性:%
 \[
 ...
 \]%
 可以看出这是三维旋转群$O(3)$的Lie代数, 也是二维幺正幺模矩阵的群$SU(2)$的Lie代数, 它们在杨振宁和Mills的原始非阿贝尔理论中被用作基. 
 以同样的方式继续下去, 当生成元个数为4, 5, 6或7时, 单Lie代数不存在, 生成元个数为8时有一个单Lie%
 ...
 g_{ma,nb}=g_{m}^{-2}\updelta_{mn}\updelta_{ab}  \label{15.2.5}
 \end{equation}%
 ...
 \end{equation}%
 消掉常数$g_{m}^{-2}$, 但这样一来, 为了保持$D_{\mu }\psi $和$F_{\alpha}{}^{\mu\nu}$的公式(\ref{15.1.10})和(\ref{15.1.13})不变, 
 ...
 \begin{equation}
 g_{ab}=\updelta_{ab}\:,   \label{15.2.9}
 \end{equation}%
 但这样一来, 对于每个单子代数或$U(1)$子代数, 变换矩阵$t_{\alpha }$和结构常数$C_{\alpha \beta \gamma }$就要包%
 ...
 这样的项将被某些非常大的质量项的负幂次压得很低. 由于这个原因, 描述弱作用, 电磁作用和强作用的标准模型的拉格朗日量的一般形式为(\ref{15.3.1}).
 ...
 \partial _{\nu }\mathscr{J}_{\alpha}{}^{\nu}=0\:,   \label{15.3.4}
 \end{equation}%
 这一点既可以从$\psi $的欧拉-拉格朗日方程和不变性条件(\ref{15.2.2})中看到, 另一种更简单的方法是从场方程(\ref{15.3.2})中直接得出.
 ...
 \section{量子化}
 我们现在开始量子化前两节描述的规范理论. 拉格朗日密度取为(\ref{15.3.1})的形式:
 \begin{equation}
 ...
 因为我们是在轴规范下进行量子化的, 而轴规范下没有这个问题, 所以Gribov多义性在这里不会影响我们, 而对于其他规范, 例如Lorentz规范, 
 ...
 \label{15.4.5}
 \end{equation}%
 场$A_{\alpha 0}$不是独立的正则变量, 而是通过约束(\ref{15.4.3})由其他变量定义的. 为了看清这一点, 注意到``电''场场强%
 ...
 (我们采用了求和约定, 指标$i,j$等要对1和2求和.) 应该注意的是, 物质场$\psi _{\ell }$的正则共轭是%
 \begin{equation}
 ...
 依照9.2节所导出的一般规则, 利用这一哈密顿密度, 我们现在可以通过计算$A_{\alpha
 i}$, $\Pi _{\alpha i}$, $\psi _{\ell }$和$\pi _{\ell }$在权重因子$\exp(\mi I)$下的路径积分来计算矩阵元, 这里
 \begin{equation}
 I=\int \dif^{4}x\, \Big[ \Pi _{\alpha i}\partial _{0}A_{\alpha i}+\pi _{\ell
 }\partial _{0}\psi _{\ell }-\mathscr{H}+\epsilon \,\text{项}\,\Big]\:,  \label{15.4.13}
 \end{equation}%
 在$I$中, ``$\epsilon $项''仅用来给出传播子分母中正确的无限小虚项. (参看9.2节.) 
 我们注意到方程(\ref{15.4.7})和(\ref{15.4.9})给出的$A_{\alpha }{}^{0}$是正则变量的泛函, 而该泛函关于$\Pi
 _{\alpha i}$和$\pi _{\ell }$是{\kai{线性}}的.
 那么, 对方程(\ref{15.4.11})的观察表明, (假定$\mathscr{L}_{\mathrm{M}}$%
 至多是$D_{\mu }\psi $的平方)完整作用量(\ref{15.4.13})的被积函数关于$\Pi _{\alpha i}$和$\pi _{\ell }$至多是二次的. 
 因此, 通过通常的高斯积分规则, 我们可以算出对这些正则``动量''的路径积分. 这一步骤的麻烦是方程(\ref{15.4.13})中$\Pi _{\alpha i}$的二阶项系数是$A_{\alpha i}$的函数, 所以高斯积分将会带来一个棘手的依赖场的行列式因子. 此外, 这时的整个体系看起来没有可能是Lorentz不变的.
 
 我们不再沿着这条思路继续下去了, 取而代之, 我们将采用一个类似于在9.6%
 节的电动力学路径积分中使用过的技巧. 注意, 如果我们暂且认为$A_{\alpha 0}$是独立变量, 那么作用量(\ref%
 {15.4.13})显然是$A_{\alpha 0}$的二次型, 其中二阶项$A_{\alpha 0}(x)A_{\beta 0}(y)$的系数等于不依赖场的核$(\partial _{3})^{2}\updelta^{4}(x-y)$. 正如我们在第九章附录中所看到的, 这种对$A_{\alpha 0}(x)$的高斯积分, 除了一个常数因子外, 等于被积函数在指数变量稳定``点''的值. 但是, 这里作用量变分导数是
 \[
 \frac{\updelta I}{\updelta A_{\alpha 0}}=-\frac{\partial \mathscr{H}}{\partial
 ...
 的二次型, 其中二阶项$\Pi _{\alpha i}(x)\Pi _{\beta j}(y)$的系数由不依赖场的核$\frac{1}{2}\updelta^{4}(x-y)\updelta_{ij}$给出. 假定这对于物质场$\pi _{\ell }$同样成立, 通过取$\pi_{\ell }$和$\Pi _{\alpha i}$等于作用量(\ref{15.4.1})的驻定``点'', 
 对$\pi_{\ell }$和$\Pi _{\alpha i}$的路径积分可以计算至只差一个常数因子, $%
 \pi _{\ell }$和$\Pi _{\alpha i}$的驻定``点''是:%
 \begin{align*}
 0 &=\frac{\updelta I}{\updelta \pi _{\ell }}=\partial _{0}\psi _{\ell }-\frac{%
 \partial \mathscr{H}_{\mathrm{M}}}{\partial \pi _{\ell }}\:,  \\
 0 &=\frac{\updelta I}{\updelta \Pi _{\alpha i}}=\partial _{0}A_{\alpha i}-\Pi
 ...
 \begin{equation}
 \prod_{x,\alpha }\updelta \Big( A_{\alpha 3}(x)\Big) \label{15.4.15}
 \end{equation}%
 ...
 \}\prod_{x,\alpha }\updelta (A_{\alpha 3}(x))\:,   \label{15.4.16}
 \end{align}%
 ...
 \Lambda _{\alpha }(x)$. 只需证明这对单位元附近变换参量为无限小$\lambda _{\alpha }(x)$的规范变换成立就足够了.
 %这对证明单位元附近的变换, 如无限小的变换参量$\lambda _{\alpha }(x)$, 就已经足够了.
 在这种情况下
 \[
 ...
 其中$\mathscr{N}$是``矩阵'':%
 \[
 \mathscr{N}_{\alpha \mu x,\beta \nu y}=\frac{\updelta A_{\lambda }%
 {}_{\alpha\mu}(x)}{\updelta A_{\beta \nu }(y)}=\updelta^{4}(x-y)\,\updelta_{\mu
 }^{\nu }\,\Big[ \updelta_{\alpha \beta }+C_{\alpha \beta \gamma }\lambda
 _{\gamma }(x)\Big] \:. 
 \]%
 因为迹$C_{\alpha \alpha \gamma }$为零, 所以$\mathscr{N}$的行列式精确到$\lambda _{\gamma }$的一阶是1.
 ...
 \begin{equation}
 \mathscr{F}_{\alpha x, \beta y}[\phi] \equiv \left.\frac{\updelta f_{\alpha}[\phi_{\lambda};x]}{\updelta \lambda_{\beta}(y)}  \right \vert _{\lambda=0} \:.  \label{15.5.3}
 \end{equation}
 (与通常函数的泛函或泛函的泛函的记法一致, $B\Big[f[\phi ]\Big]$表示的是: 保持写出的变量------函数$\phi_{n}(x)$-----不变, $B[f[\phi]]$依赖于$f_{\alpha}[\phi]$对未标出的变量$\alpha $和$x$的所有值的取值.) 方程(\ref{15.5.1})并不代表方程(\ref{15.4.16})最大可能的推广; 在15.7节我们将看到, 出于某些原因, 需要并存在进一步的推广. 这里我们从方程(\ref{15.5.1})出发, 是因为它将有助于推动15.7节的形式体系, 并且在最方便的规范下足以用来处理非阿贝尔规范理论.
 
 我们现在必须验证路径积分(\ref{15.4.16})确实是方程(\ref{15.5.1})的一个特殊情况. 在方程(\ref{15.4.16})中, 
 场$\phi _{n}(x)$由$A_{\alpha \mu }(x)$和物质场$\psi_{\ell}(x)$构成, 并且
 \begin{align}
 &f_{\alpha }[A,\psi ;x] =A_{\alpha 3}(x)\:,   \label{15.5.4} \\
 &B[f] =\prod_{x,\alpha }\updelta \Big( f_{\alpha }(x)\Big) \:,  \label{15.5.5} \\
 ...
 \[
 A_{\lambda\alpha}^{3}(x) = \partial_{3}\, \lambda_{\alpha}(x) = \int \dif^{4}y\: \lambda_{\alpha}(y)\, \partial_{3}\updelta^{4}(x-y) \:, 
 \]
 使得方程(\ref{15.5.3})在这里是不依赖场的``矩阵''
 \[
 \mathscr{F}_{\alpha x ,\beta y}[\phi] = \updelta_{\alpha \beta} \,\partial_{3} \updelta^{4}(x-y) \:. 
 \]
 因此, 方程(\ref{15.5.1})中的行列式在这一规范下也不依赖场. 正如第9章所讨论过的, 
 泛函积分中不依赖场的因子仅影响期望值和$S$-矩阵的真空涨落部分(\textcolor{red}{讨论}), 因而与$S$-矩阵元连通部分的计算无关.
 
 非阿贝尔理论的泛函积分(\ref{15.4.16})可以视为一般路径积分(\ref{15.5.1})的一个特殊情况, 其中的关键之处在于, 在这一%
 形式下我们可以自由地改变规范. 特别地, 我们有一个定理, 即{\kai{积分(\ref{15.5.1})实际上(在一个很宽泛的限制下)独立于规范固定泛函$f_{\alpha}[\phi;x]$, 它仅通过一个不相关的常数因子依赖于泛函$B[f]$的选择.}}
 ...
 并且还没用到我们关于规范不变性的假定.) 已假定了测度$\prod\dif\phi $乘以泛函$\mathscr{G}[ \phi ]$有规范不变性(\ref{15.5.2}), 
 现在用这一点将其重写为
 \begin{equation}
 ...
 \mathscr{F}_{\alpha x,\beta y}[\phi_{\Lambda}]= \frac{\updelta f_{\alpha }[(\phi
 _{\Lambda })_{\lambda };x]}{\updelta \lambda ^{\beta }(y)}\biggr\vert
 _{\lambda =0}\:.   \label{15.5.12}
 \end{equation}%
 我们假定这些变换构成群; 就是说, 先进行参量为$\Lambda ^{\alpha }(x)$的规范变换再进行参量为$\lambda ^{\alpha }(x)$的规范变换%
 后的结果可以写成只``乘''一次参量为$\tilde{\Lambda}^{\alpha }(x;\Lambda,\lambda )$的规范变换作用后的结果,%
 \begin{equation}
 ...
 \end{equation}%
 其中\begin{equation}
 \mathscr{I}_{\alpha x,\gamma z}[\phi ,\Lambda ]\equiv \left. \frac{\updelta
 f_{\alpha }[\phi _{\tilde{\Lambda}};x]}{\updelta \tilde{\Lambda}^{\gamma }(z)}%
 \right\vert _{\tilde{\Lambda}=\Lambda }=\frac{\updelta f_{\alpha }[\phi
 _{\Lambda };x]}{\updelta \Lambda ^{\gamma }(z)}  \label{15.5.15}
 \end{equation}%
 以及
 \begin{equation}
 \mathscr{R}^{\gamma z}{}_{\beta y}[ \Lambda ]= \frac{\updelta
 \tilde{\Lambda}^{\gamma }(z;\Lambda ,\lambda )}{\updelta \lambda ^{\beta }(y)}%
 ...
 \label{15.5.19}
 \end{align}%
 它显然不依赖$\phi$. (读者可以认出方程(\ref{15.5.18})给出了群参数空间上的不变(Haar)测度.) 最终我们有\begin{equation}
 ...
 \end{equation}%
 这个结果显然与我们对$f_{\alpha }[\phi ;x]$的选择无关, 它已经被简化成单变量的积分, 并且它仅通过常数$C$依赖$B[f]$, 这正是我们要证明的.
 
 在继续应用这个定理之前, 我们应该停下来说明一下推导中的一个技巧. 由于同一个原因, 
 方程(\ref{15.5.20})的分子和分母中的积分没有明确的定义. 
 因为假定$\mathscr{G}[ \phi ]$是规范不变的, 所以它对$\phi$的积分不可能收敛; 
 在将$\phi $变成$\phi _{\lambda }$的所有可能的$\lambda _{\alpha}(x)$的轨道上, $\mathscr{G}[\phi]$保持不变.
 类似的, 分母中的积分也是发散的, 这是因为$\rho[ \Lambda ]\prod \dif\Lambda $就是通常的在群上积分的不变体积元,
 并且它沿着``轨道''$\Lambda \rightarrow \tilde{\Lambda}(\Lambda ,\lambda )$也保持不变. 通过在一个有限大的时空晶格内表述该理论, 
 ...
 我们可以推测, 随着格点的数目趋于无穷, 这一发散在方程(\ref{15.5.20})右边的分子和分母之间抵消了.
 
 现在进入主题. 我们已经看到, 轴规范下的真空期望值(\ref{15.4.16})由一般形式的路径积分(\ref{15.5.1}%
 )给出. 有了上面的定理后, 我们得到结论
 \begin{align}
 ...
 \end{align}%
 其中对$f_{\alpha }[A,\psi ;x]$和$B[f]$的选择(几乎)是任意的. 因此, 
 我们现在可以在一个更方便的规范下自由地使用方程(\ref{15.5.21})推导Feynman规则.
 ...
 最简单的Lorentz不变的规范固定函数$f_{\alpha }$与电动力学中的选择相同:%
 \begin{equation}
 ...
 }{\partial x^{\lambda }\partial y_{\lambda }}\,\updelta^{4}(x-y)\updelta_{\alpha
 \beta } \\
 &\quad-\left( 1-\frac{1}{\xi }\right)\, \frac{\partial ^{2}}{\partial x^{\mu
 }\partial y^{\nu }}\,\updelta^{4}(x-y)\updelta_{\alpha \beta }+\epsilon\, \text{项} \\
 &=(2\uppi )^{-4}\updelta_{\alpha \beta }\int \dif^{4}p\left[ \eta _{\mu \nu
 ...
 泛函(\ref{15.5.22})振荡得非常剧烈, 所以这个泛函的作用就像$\updelta$-函数, 给泛函积分强加了Landau规范条件$\partial _{\mu }A^{\mu }=0$, 
 它自然地给出了满足相应条件$\partial ^{\mu }\Delta _{\alpha \mu ,\beta \nu }=0$的传播子. 对不为零的$\xi $值, 
 泛函$B[f]$并不选出那些使得$A_{\alpha \mu }$满足任何特定规范条件的规范场, 但是通常将传播子(\ref{15.5.25})称为``广义Feynman规范''或``广义$\xi$-规范''下的传播子.
 一个很好的计算物理振幅的策略是, 先保持$\xi$任意, 然后在计算的最后检验结果是不是$\xi $-无关的.
 
 附加一个条件后, Feynman规则现在是显然的: 顶点的贡献可以从原始拉格朗日量$\mathscr{L}$的相互作用项中读出来, 而规范场%
 ...
 \mi(2\uppi )^{4}\updelta^{4}(p+q+k)[-\mi C_{\alpha \beta \gamma }]\Big[ p_{\nu
 ...
 那么这种顶点对被积函数的贡献是
 \begin{align}
 &\mi (2\uppi )^{4}\updelta^{4}(p+q+k+\ell )\times \Big[ -C_{\epsilon \alpha
 ...
 我们现在来考察方程(\ref{15.5.21})中的因子$\operatorname{Det}\mathscr{F}$对非阿贝尔规范场论Feynman规则的影响. 
 为了能将这个因子产生的影响处理成对Feynman规则的修正, 回顾我们在9.5节所证明的一个结果, 即,
 任何矩阵$\mathscr{F}_{\alpha x,\beta y}$的行列式都可以表示为一个路径积分
 \begin{equation}
 ...
 并且(\ref{15.6.1})中的比例常数与场无关. (为了能再次给出因子$\operatorname{Det} \mathscr{F}$, 
 我们必须将场变量$\omega _{\alpha }$和$\omega _{\alpha }^{\ast }$选成费米场变量; 如果我们将这些场变量选成玻色的, 那么路径积分(\ref{15.6.1})就会正比于$(\operatorname{Det} \mathscr{F})^{-1}$.) 场$\omega _{\alpha }^{\ast}$和$\omega _{\alpha }$不一定要是复共轭的, 我们会在15.7节看到, 由于某些原因, 我们甚至需要假定$\omega _{\alpha }^{\ast }$和$\omega _{\alpha }$是无关的{\kai{实}}变量. 因子$\operatorname{Det} \mathscr{F}$的全部影响等同于把$I_{GH}[\omega,\omega^{\ast}]$纳入到全部的有效作用量中, 
 并对``场'' $\omega $和$\omega ^{\ast }$积分. 即, 对于任意的规范固定函数$f_{\alpha }(x)$,%
 \begin{align}
 ...
 对方程(\ref{15.6.2})的观察表明, 这个作用量有一个被称作``鬼数''的守恒量:  
 ...
 \]%
 使得\begin{align}
 &\mathscr{F}_{\alpha x,\beta y} =\left. \frac{\updelta \partial _{\mu
 }A_{\lambda \alpha}{}^{\mu}(x)}{\updelta \lambda _{\beta }(y)}\right\vert
 _{\lambda =0}  \nonumber \\
 &=\square \,\updelta^{4}(x-y)+C_{\alpha \gamma \beta }\frac{\partial }{%
 \partial x^{\mu }}\,\Big[ A_{\gamma}{}^{\mu}(x)\updelta^{4}(x-y)\Big] \text{
 .}  \label{15.6.9}
 \end{align}
 这正是(\ref{15.6.5})的形式, 其中\begin{align}
 (\mathscr{F}_{0})_{\alpha x,\beta y} &=\square \updelta^{4}(x-y)\,\updelta
 _{\alpha \beta }\:,   \label{15.6.10} \\
 (\mathscr{F}_{1})_{\alpha x,\beta y} &=-C_{\alpha \beta \gamma }\frac{%
 \partial }{\partial x^{\mu }}\Big[ A_{\gamma}{}^{\mu}(x)\updelta^{4}(x-y)%
 \Big] \:.   \label{15.6.11}
 \end{align}%
 由方程(\ref{15.6.6})和(\ref{15.6.10})中, 我们看到鬼场传播子为
 \begin{equation}
 \Delta _{\alpha \beta }(x,y)=\updelta_{\alpha \beta }(2\uppi )^{-4}\int
 \dif^{4}p\:(p^{2}-\mi\epsilon )^{-1}\,e^{\mi p\cdot (x-y)}\:,   \label{15.6.12}
 \end{equation}%
 所以,鬼场的行为在这一规范下就像零质量的无自旋费米子, 它按照规范群的伴随表示变换. 
 ...
 \begin{equation}
 \mi(2\uppi )^{4}\updelta^{4}(p+q+k)\:\times \:\mi p_{\mu }C_{\alpha \beta \gamma }\text{
 .}  \label{15.6.14}
 \end{equation}%
 鬼粒子围绕着一个圈传播, 而圈上的每一顶点与单个矢量玻色子线相连, 像通常对费米场变量所做的那样, 
 每一个圈都要补充一个额外的负号.
 ...
 \omega $和$\omega ^{\ast }$的量纲, 它们是质量的一次方, 与普通的标量场以及规范场相同.) 然而, 这里的可重正性并不仅仅是幂次计数; 同样必要的是, 存在一个抵消项吸收所有的发散. 在下一节, 我们将考察一个极为重要的对称性, 在17.2节我们将用这个对称性证明非阿贝尔场论在这一意义下确实是可重正的, 除此之外, 这个对称性甚至可以取代我们一直讨论的Faddeev-Popov-De Witt方法.
 
 \section{BRST对称性}
 
 尽管前两节所描述的Faddeev-Popov-De Witt方法使理论的Lorentz不变性变得显然, 但它仍然基于一个规范的选取, 因而, 它自然地掩盖了理论本质上的规范不变性. 在尝试证明理论的可重正性时, 这是一个严重的问题------规范不变性限制了拉格朗日量中用来吸收紫外发散的抵消项的形式, 但是一旦我们选定了规范, 我们如何知道规范不变性是否仍然会限制着无穷大所产生的方式?
 ...
 \end{equation}%
 这个作用量对应的是(\ref{15.5.21})中选择的规范固定泛函:%
 \begin{equation}
 ...
 \end{equation}
 做对$h_{\alpha }$ (通常称为``Nakanishi-Lautrup''场\cite{11a}), 以及物质场, 规范场, 鬼场和反鬼场的路径积分.
 
 修正作用量不是规范不变的——甚至, 如果我们要在路径积分中能够用它的话, 它不是规范不变的反而更好. 然而它在``BRST''对称变换下不变, 该对称变换由一个无限小常数$\theta $参数化, $\theta $与$\omega _{\alpha }$, $\omega _{\alpha }^{\ast }$%
 以及所有的费米物质场{\kai{反对易}}. 对于给定的$\theta $, BRST变换是\begin{align}
 \updelta_{\theta }\psi  &=\mi t_{\alpha }\theta \omega _{\alpha }\psi \:, 
 \label{15.7.7} \\
 \updelta_{\theta }A_{\alpha \mu } &=\theta D_{\mu }\omega _{\alpha }=\theta
 [ \partial _{\mu }\omega _{\alpha }+C_{\alpha \beta \gamma }A_{\beta
 \mu }\omega _{\gamma }]\:,   \label{15.7.8} \\
 \updelta_{\theta }\omega _{\alpha }^{\ast } &=-\theta h_{\alpha }\:, 
 \label{15.7.9} \\
 \updelta_{\theta }\omega _{\alpha } &=-\tfrac{1}{2}\theta\, C_{\alpha \beta
 \gamma }\omega _{\beta }\omega _{\gamma }\:,   \label{15.7.10} \\
 \updelta_{\theta }h_{\alpha } &=0\:.   \label{15.7.11}
 \end{align}%
 ...
 们用\begin{equation}
 \updelta_{\theta }F\equiv \theta sF  \label{15.7.12}
 \end{equation}%
 定义$sF$, 那么\footnote{在BRST对称性的原始工作中, 泛函$B[f]$仍取作(\ref{15.7.4})的形式, 从而使方程(\ref{15.7.9})中的$h_{\alpha}$被替换成了$-f_{\alpha}/\xi$, 并且, 仅当作用在$\omega_{\alpha}$以及规范场和物质场的函数上时, BRST变换才是幂零的, 而对于$\omega_{\alpha}^{\ast}$的函数则不是.}
 \begin{equation}
 \updelta_{\theta }(sF)=0\:,   \label{15.7.13}
 \end{equation}%
 或等价地,
 \begin{equation}
 ...
 当$\updelta_{\theta }$作用在单个场上时, 验证这一幂零性是直接的. 首先, 作用在物质场上,%
 \begin{align*}
 \updelta_{\theta }s\psi  =\mi t_{\alpha }\,\updelta_{\theta }(\omega _{\alpha
 ...
 下一步, 作用在规范场上, 我们有
 \begin{align*}
 \updelta_{\theta }sA_{\alpha \mu } &=\updelta_{\theta }D_{\mu }\omega
 _{\alpha } \\
 &=\partial _{\mu }\updelta_{\theta }\omega _{\alpha }+C_{\alpha \beta \gamma
 }\updelta_{\theta }A_{\beta \mu }\omega _{\gamma }+C_{\alpha \beta \gamma
 }A_{\beta \mu }\updelta_{\theta }\omega _{\gamma } \\
 ...
 \begin{align*}
 \updelta_{\theta }s\omega _{\alpha } &=-\tfrac{1}{2}C_{\alpha \beta \gamma
 }\updelta_{\theta }(\omega _{\beta }\omega _{\gamma }) \\
 ...
 \updelta_{\theta }(\phi _{1}\phi _{2})=\theta (s\phi _{1})\phi _{2}+\phi
 ...
 \]%
 正如我们所看到的, 由于$\updelta_{\theta }(s\phi _{1})=\updelta_{\theta }(s\phi _{2})=0$, BRST%
 变换作用在$s(\phi _{1}\phi _{2})$上的结果是
 \[
 \updelta_{\theta }s(\phi _{1}\phi _{2})=(s\phi _{1})\theta (s\phi _{2})\pm
 \theta (s\phi _{1})(s\phi _{2})\:. 
 \]%
 然而, $s\phi$总有一个与$\phi$相反的统计, 所以将右边第一项中的$\theta$移至左边会带来一个符号因子$\mp $:%
 \[
 \updelta_{\theta }s(\phi _{1}\phi _{2})=\theta \Big[ \mp (s\phi _{1})(s\phi
 _{2})\pm (s\phi _{1})(s\phi _{2})\Big] =0\:. 
 \]%
 这样继续作下去, 我们会看到, BRST变换作用在任意时空点的场的任意乘积上都是幂零的:%
 \[
 \updelta_{\theta }\,s(\phi _{1}\phi _{2}\phi _{3}\cdots )=0\:. 
 \]%
 任何泛函$F[\phi ]$都能写成这种乘积再乘上c-数系数的多重积分之和, 所以类似地有
 \begin{equation}
 \updelta_{\theta }\,sF[\phi ]=\theta ssF[\phi ]=0\:.   \label{15.7.20}
 \end{equation}%
 这样就补全了BRST变换幂零性的证明.
 ...
 \begin{equation}
 \updelta_{\theta }\int \dif^{4}x\:\mathscr{L}=0\:.   \label{15.7.22}
 \end{equation}%
 为了计算BRST变换对作用量(\ref{15.7.6})其他部分的影响, 要注意到它对规范固定函数的影响就是规范变换(\ref{15.7.21}), 从而
 \begin{align*}
 \updelta_{\theta }f_{\alpha }[x;A,\psi ] &=\int \left. \frac{\updelta
 f_{\alpha }[x;A_{\lambda },\psi _{\lambda }]}{\updelta \lambda ^{\beta }(y)}%
 ...
 或者利用(\ref{15.7.3})%
 \begin{equation}
 \updelta_{\theta }f_{\alpha }[x;A,\psi ]=\theta \Delta _{\alpha }(x;A,\psi,\omega )\:.   \label{15.7.23}
 \end{equation}%
 (注意, $\mathscr{F}$是一个玻色量, 所以在这里将$\theta $%
 移至左边时没有符号变化.) 另外, 回忆起$\updelta
 _{\theta }\omega _{\alpha }^{\ast }=-\theta h_{\alpha }$和$\updelta_{\theta }h_{\alpha }=0$. 因此, 在``新''作用量(\ref{15.7.6})的被积函数中, 除$\mathscr{L}$以外的项可以写成%
 \begin{equation}
 ...
 在某种意义上, 这一结果的逆命题也是成立的: 我们会在17.2节看到, 对于服从BRST不变性以及拉格朗日量(\ref{15.7.25})的其他对称性的可重正拉格朗日量, 除了各种常系数的值会有一个变化外, 必须取方程(\ref{15.7.25})的形式. 但这还不足以确立这些理论的可重正性. BRST对称变换以非线性的方式作用在场上, 在这种情况下,
 ...
 后的那部分中. 核在模掉任意变换的像后, 称作构成了该变换的{\kai{上同调}}. 还有另一种说法, 一个规范理论的物理部分可以等同为BRST算符的上同调.\cite{12} 一个基本的物理要求是, 物理态之间的矩阵元应该不依赖我们对规范固定函数$f_{\alpha }$的选择, 换句话说, 矩阵元应该不依赖方程(\ref{15.7.25})中泛函$\Psi $的选择. 任意矩阵元$\langle \alpha |\beta
 \rangle $在$\Psi $变化$\tilde{\updelta}\Psi$时的变化是
 \begin{equation}
 \tilde{\updelta}\langle \alpha |\beta \rangle =\mi\langle \alpha |\tilde{\updelta}%
 I_{\mathrm{NEW}}|\beta \rangle =\mi\langle \alpha |s\tilde{\updelta}\Psi |\beta \rangle
 \:.   \label{15.7.27}
 \end{equation}
 (我们在这里使用波浪符是为了将规范固定函数中的这种任意变化与BRST变换或规范变换区分开.) 我们可以引入一个费米的BRST ``核'' $Q$, 
 它被定义成, 对任意场算符$\Phi$,
 \[
 \updelta_{\theta} \Phi = \mi[\theta\,Q , \Phi] = \mi\theta\,[Q,\Phi]_{\mp} \:, 
 \]
 换句话说,
 \begin{equation}
 ...
 \begin{equation}
 \tilde{\updelta}\langle \alpha |\beta \rangle =\langle \alpha |[Q,\tilde{\updelta%
 }\Psi ]|\beta \rangle \:.   \label{15.7.30}
 \end{equation}%
 为了使其对$\Psi $的所有变化$\tilde{\updelta}\Psi$都为零, 必须有
 \begin{equation}
 \langle \alpha |Q=Q|\beta \rangle =0\:.   \label{15.7.31}
 \end{equation}%
 因此, 物理态在幂零算符$Q$的核中. 两个物理态, 若它们仅相差$Q$的像中的一个态矢, 即, 形式为$%
 Q|\cdots \rangle $的态矢, 显然, 它们与所有其他物理态之间的矩阵元是相同的, 因而在物理上是等价的.
 因此, {\kai{独立}}的物理态对应于$Q$的核模掉$Q$的像后中的态------即, 它们对应于$Q$的上同调.
 
 为了看到这在实际情形中是如何运作的, 我们来考察一个纯电动力学的简单例子.\footnote{由于电动力学中的结构常数为零, 方程(\ref{15.6.11})和(\ref{15.6.7})表明, 这里鬼场不与其他场耦合. 然而, 就用BRST对称性识别物理态这一点而言, 电动力学提供了一个很好的例子. 确实如此, 在分析``入''态和``出''态上的物%
 ...
 b^{\ast }$和$c^{\ast }$也不是$c$和$b$的共轭. 然而, 因为$A^{\mu }(x)$是厄米的, 那么如果$Q$是厄米的, $\omega (x)$也将是厄米的.}%
 \begin{align}
 ...
 }A_{\alpha}{}^{\mu }$的可重正Yang-Mills理论, 这足够了, 但对于更普遍的情况, 作用量仅仅关于鬼场是双线性的还不够. 例如, 我们会在17.2节看到, 在其他规范下, 对于有4条外鬼线的圈图中的紫外发散,
 可重正Yang-Mills理论的拉格朗日密度中需要$\omega ^{\ast}\omega ^{\ast }\omega \omega $项来充当它的抵消项.
 
 幸运的是, Faddeev-Popov-De Witt体系仅仅代表一种生成一类等价拉格朗日量的方法, 这里说的拉格朗日量等价是指它们所产生的幺正$S$-矩阵相同. BRST体系提供了一个更加普遍的方法, 它完全省去了Faddeev-Popov-De Witt体系. 在这一方法中, 将作用量取为物质场, 规范场, $\omega ^{A}$场, $\omega ^{\ast A}$场以及$h^{A}$场的最一般的定域泛函, 但满足鬼数为零, 并且在BRST变换(\ref{15.7.7})---(\ref{15.7.11})以及该理论的其他所有整体对称性下不变. (对于可重正理论, 还要限制拉格朗日密度为量纲小于等于4的算符, 但这个限制在下面的讨论中没有任何影响.) 我们会在下一节证明, 在一个比Yang-Mills理论更普遍的情形下, 这类作用量的最普遍形式是两项之和, 其中第一项仅是物质场和规范场(统称为$\phi $)的泛函, 而第二项由BRST算符$s$作用在鬼数为$-1$的任意泛函$\Psi $上产生:%
 \begin{equation}
 ...
 通过与此前相同的讨论, 我们得到这样的结论: 对于被BRST算符$Q$所湮灭的态, 它的$S$-矩阵元与方程(\ref{15.7.40})中$\Psi $的选择无关, 所以只要可以选择$\Psi$使鬼场退耦, 那么鬼场在所有情形下都会退耦. 在Yang-Mills理论中, 轴规范下的量子化给出了一个这样的$\Psi $, 所以在这样的理论中, 不仅是那些如(\ref{15.7.26})那样由Faddeev-Popov-De Witt体系给出的选择, 
 对泛函$\Psi [ \phi ,\omega ,\omega ^{\ast },h]$的任何选择都会使鬼场退耦.
 
 我们可以更进一步, 并且完全摆脱对非Lorentz不变规范下的正则量子化的依赖, 例如对轴规范下正则量子化的依赖. 再一次, 取作用量为规范场, 物质场, $\omega ^{A}$场, $\omega ^{\ast A}$场以及$h^{A}$场的最一般泛函, 满足鬼数为零, 且在BRST变换(\ref{15.7.7})---(\ref{15.7.11})以及该理论的包括Lorentz不变性在内的其他任何整体对称性下不变. 从作用量的BRST不变性中,
 我们可以推断出存在一个守恒的幂零BRST生成元$Q$. 而把鬼场和反鬼场取做厄米场时, $Q$也将是厄米的. 像前面一样, 物理态的空间定义为被$Q$湮灭的态构成的空间, 其中, 如果两个态的差是$Q$作用于另一个态, 那么视这两个态等价. 已经证明了, 对于Yang-Mills理论, 这一空间没有鬼和反鬼且有一个正定的范数, 并且这一空间中的$S$-矩阵是幺正的.
 
 这一手续称为\textit{BRST}{\kai{量子化}}. 它已经被拓展到具有其他定域对称性的理论, 例如广义相对论和弦论. 不幸的是, 到现在为止, 
 看起来还必须要在每一种情形下分别证明BRST上同调无鬼以及作用在该空间上的$S$-矩阵幺正. 这些证明中的关键是, 对于每一个负范的自由度, 例如Yang-Mills理论中规范场的时间分量, 都存在一%
 个定域对称性使这一自由度被变换掉了.
 ...
 下一章会介绍外场方法, 在第17章, 我们将用这一方法, 结合BRST对称性, 完成非阿贝尔规范场论可重正性的证明.
 ...
 我们处理在如下的无限小变换
 \begin{equation}
 \phi ^{r}\rightarrow \phi ^{r}+\epsilon ^{A}\updelta_{A}\phi ^{r}  \label{15.8.1}
 \end{equation}%
 ...
 以方程(\ref{15.8.1})的记法, 变换(\ref{15.1.9})中的变分$\updelta_{A}\phi ^{r}$为
 \[
 \updelta_{\beta \, y}\phi ^{\mu\, \alpha \,x}=\updelta_{\alpha}^{\beta }\frac{\partial }{\partial x^{\mu }}\updelta ^{4}(x-y)
 +C^{\beta}{}_{\gamma \alpha }\phi ^{\mu\, \gamma\, x}\updelta^{4}(x-y)\:. 
 \]%
 ...
 \:\me^{\mi I[\phi ]}\:B[f[\phi ]]\, \operatorname{Det} (\updelta_{A}f_{B}[\phi ])\:V[\phi ]\:,  \label{15.8.2}
 \end{equation}%
 其中$V[\phi ]$是在规范变换(\ref{15.8.1})下不变的$\phi^{r}$的任意泛函; $f_{A}[\phi ]$是一组$\phi ^{r}$的规范固定泛函\footnote{%
 我们用相同的字母$A,B$等来标记$f_{A}$和规范变分$\updelta_{A}$, 是为了强调规范固定泛函的数目要与独立规范变换的数目相同. 然而, 在某些情形下, 例如在弦理论中, 
 即便指标$a$与规范变分$\updelta_{A}$上的指标$A$的取值完全不同, 只要它们所取的值``个数相同'', 就会很自然地采用$f^{a}$. 只要我们能够定义$f_{A}=c_{Aa}f^{a}$, 
 其中$c_{Aa}$是场无关且非奇异的, 那么在现在的体系中就不需要作出改变.}, 对它的选择要使``矩阵'' $\updelta_{A}f_{B}[\phi ]$有一个非零的行列式, 
 而$B[f]$是$f_{A}$的泛函, 它的选择多少有些任意(例如$\prod_{A}\updelta(f_{A})$.) 常数$\Omega$是规范群的体积元, 
 而常数$C$ (像方程(\ref{15.5.19})那样)的定义为
 \begin{equation}
 C\equiv \int [\dif f]\:B[f]\:.   \label{15.8.3}
 \end{equation}%
 我们已经看到, 方程(\ref{15.8.2})在规范理论中的重要性就在于, 它告诉我们右边的积分与规范固定泛函$f_{A}$如何选择无关,
 而仅通过常数$C$依赖于$B[f]$. 在这里, 我们可以给通常无限大的规范群体积$\Omega $赋予某些含义, 就像我们在有限时空格点上的规范理论中所做的那样, 这样, 
 ...
 行列式可以表示成对费米c-数场\footnote{在弦论和其他地方, 通常会发现鬼场$\omega ^{\ast A}$和$\omega ^{A}$分别%
 写成了$b^{A}$(或$b_{A}$)和$c^{A}$.} $\omega
 ^{\ast A}$和$\omega ^{A}$的积分:%
 \begin{equation}
 \operatorname{Det} (\updelta _{A}f_{B}[\phi ])\propto \int [\dif\omega ^{\ast }]\,[\dif\omega
 ]\:\exp \Big(\mi\omega ^{\ast B}\omega ^{A}\updelta _{A}f_{B}\Big)\:, 
 \label{15.8.5}
 \end{equation}%
 ...
 &\int [\dif\phi ]\:\exp \Big( \mi I[\phi]\Big) \:B[f[\phi ]]\:\operatorname{Det} (\updelta_{A}f_{B}[\phi ])\:V[\phi ]  \nonumber \\
 ...
 ]+\omega ^{\ast B}\omega ^{A}\updelta_{A}f_{B}[\phi ]\:.   \label{15.8.7}
 \end{equation}%
 
 在15.6节中曾提到过, 对所有$\phi ^{r}$的积分也包含了那些仅相差规范变换(\ref{15.8.1})的$\phi ^{r}$, 而我们可%
 ...
 那么规范变换(\ref{15.8.1})也必须全是独立的. 这正是Yang-Mills理论中的规范变换和广义相对论中的坐标变换的情况, 但也不总是这样. 规范变换不独立的理论的经典例子是我们在8.8节曾介绍过的$p$-形式规范场. $p$-形式$A$ ($p$阶反对称张量)进行规范变换$A\to A+\dif\phi $, 其中$\phi $是$(p{-}1)$-形式, 而$%
 ...
 尽管原始对称性(\ref{15.8.1})因插入了非规范不变泛函的$B[f]$而被消除了, 但是, 在无限小BRST变换下, 新的总作用量有一个恰当对称性
 \begin{equation}
 ...
 \begin{equation}
 s=\omega ^{A}\,\updelta_{A}\phi ^{r}\frac{\updelta _{L}}{\updelta \phi ^{r}}-\frac{1%
 }{2}\omega ^{B}\omega ^{C}\,f^{A}{}_{BC}\,\frac{\updelta _{L}}{\updelta \omega ^{A}}%
 -h^{A}\,\frac{\updelta _{L}}{\updelta \omega ^{\ast A}}\:.   \label{15.8.9}
 \end{equation}%
 在方程(\ref{15.8.9})中, 下标$L$代表左微分, 它定义成如果$\updelta F=\updelta \chi \:G$, 那么$\updelta_{L}F/\updelta \chi =G$, 
 而$f^{A}{}_{BC}$是对易关系中出现的结构常数\footnote{例如, 对于作用在物质场$\psi (x)$上的规范变换, 我们有$\updelta_{\beta\, y}\psi (x)=\mi t_{\beta }\psi
 (x)\updelta ^{4}(x-y)$, 因而$\updelta_{\beta\, y}\updelta_{\gamma\, z}\psi (x)=-t_{\gamma }t_{\beta }\psi (x)\updelta ^{4}(x-y)\updelta^{4}(x-z)$.
 因此, 在这一情况下, 我们有$f^{\alpha \,x}{}_{\beta \,y\,\gamma\, z}=C^{\alpha
 }{}_{\beta \gamma }\updelta^{4}(x-y)\updelta^{4}(x-z)$.}%
 \begin{equation}
 [ \updelta_{B},\updelta_{C}]=f^{A}{}_{BC}\updelta_{A}\:. \label{15.8.10}
 \end{equation}%
 $f^{A}{}_{BC}$在非阿贝尔规范理论和弦理论中不依赖场, 尽管不总是这样, 但BRST体系并不限于这种情况. 直接的计算给出%
 \begin{align}
 s^{2} &=\frac{1}{2}\omega ^{A}\omega ^{B}\left[ \updelta_{A}\phi ^{s}\frac{%
 \updelta_{L}(\updelta_{B}\phi ^{r})}{\updelta \phi ^{s}}-\updelta_{B}\phi ^{s}%
 \frac{\updelta _{L}(\updelta _{A}\phi ^{r})}{\updelta \phi ^{s}}%
 -f^{C}{}_{AB}\updelta _{C}\phi ^{r}\right] \frac{\updelta _{L}}{\updelta \phi ^{r}}
 \nonumber \\
 &\quad-\frac{1}{2}\omega ^{B}\omega ^{C}\omega ^{D}\left[
 f^{E}{}_{BC}f^{A}{}_{DE}+\updelta _{D}\phi ^{r}\frac{\updelta _{L}f^{A}{}_{BC}}{%
 \updelta \phi ^{r}}\right] \frac{\updelta _{L}}{\updelta \omega ^{A}}\:.  \label{15.8.11}
 \end{align}%
 因此, BRST变换是幂零变换的条件等价于对易关系(\ref{15.8.10})加上一个相容性条件
 \begin{equation}
 f^{E}{}_{[BC}f^{A}{}_{D]E}+\updelta_{[ D}\phi ^{r}\left( \updelta_{L}f^{A}{}_{BC]}/\updelta \phi ^{r}\right) =0\:,   \label{15.8.12}
 \end{equation}%
 其中, 下标中的括号代表对括号内的指标$B,C$和$D$做反对称化. 按照导出通常的Jacobi恒等式的方法, 
 方程(\ref{15.8.12})可以从对易关系(\ref{15.8.10})中导出, 对于结构常数依赖于场的对称性, 它取代了Jacobi恒等式.
 ...
 作为证明这些作用量给出了物理上合理的结果中的一步, 我们现在证明如下普遍结果(曾在上一节中用到过), 
 ...
 \begin{equation}
 t\equiv \omega ^{\ast A}\frac{\updelta }{\updelta h^{A}}\:. 
 \label{15.8.16}
 \end{equation}%
 可以直接验证反对易关系\begin{equation}
 \{s,t\}=-\omega ^{\ast A}\frac{\updelta _{L}}{\updelta \omega ^{\ast A}}-h^{A}%
 \frac{\updelta }{\updelta h^{A}}\:.   \label{15.8.17}
 \end{equation}%
 那么, 用算符$\{s,t\}$作用$I_{N}$%
 ...
 都是BRST-恰当的, 也就是说, 它可以写成算符$s$作用其他某个泛函上. 因此, 完整的泛函$I$可以写成$I_{0}+s\Psi $的形式, 其中%
 \begin{equation}
 ...
 \begin{equation}
 \updelta_{\theta}\Phi = \mi [\theta\,Q , \Phi] = \mi \theta [Q, \Phi]_{\mp} \label{15.8.20}
 \end{equation}
 ...
 因此, 如果我们能找到{\kai{某个}}$\Psi $, 就像Yang-Mills理论中的轴规范, 使得在这个规范下鬼场不与其他场相互作用, 那么对于任意的%
 ...
 这一方法在弦论中运作良好, 在弦论中, 所谓的光锥量子化发挥了轴规范的作用. 但在其他理论中, 例如广义相对论, 无法通过选择坐标系使鬼场退耦. 
 ...
 但是它植根于更早的Batalin-Fradkin-Vilkovisky体系,\cite{20} 而这一体系是在哈密顿框架下导出的. 
 (已经证明这两种方案在微扰论的意义下是等价的.\cite{21}) 我们会在17.1节看到, 为了处理规范理论的重正化, Zinn-Justin\cite{22}(齐恩-朱斯坦)甚至在此之前就发展了同样的形式机制. 
 至少在三个领域中已证明这一体系的价值是无可估量的:
 
 \noindent (i)  直到现在, 我们只考虑了对称性代数在方程(\ref{15.8.10})的意义下封闭的不可约代数. 在某些理论中, 例如超引力(没有辅助场)\cite{23}, 代数是开的: 
 只有在满足场方程时, 代数才是封闭的, 这会使方程(\ref{15.8.10})中会出现正比于$\updelta I/\updelta \chi ^{n}$的项. 这样, 
 ...
 \noindent (iii)  Batalin-Vilkovisky方法提供了一个方便的途径用以分析量子效应可能对作用量对称性造成的破坏. 它在22.6节将被用于这个目的.
 ...
 而不是更通常的$\ast $, 是为了强调它与复共轭或荷共轭无关. 特别地, 反鬼场$\omega ^{\ast A}$与鬼场$\omega ^{A}$的反场$\omega _{A}^{\ddag }$%
 ...
 其中$\operatorname{gh}(\chi ^{n})$是$\chi ^{n}$的鬼数. 在最简单的情况下, 包括Yang-Mills理论和量子引力, 原本规范不变作用量$I[\phi ]$要加上%
 ...
 \begin{equation}
  0 = \frac{\updelta_{R}S}{\updelta \chi^{\ddag}_{n}}\, \frac{\updelta_{L}S}{\updelta \chi^{n}} \:,  \label{15.9.2}
 \end{equation}
 其中``$R$''和``$L$''在这里代表右微分和左微分. 为了验证它, 注意到方程(\ref{15.9.2})中反场的零阶项正好给出规范不变性条件
 \begin{equation}
 0=\Big( s\phi ^{r}\Big) \frac{\updelta_{L}I}{\updelta\phi ^{r}}=\omega^{A}\,\updelta _{A}I[\phi ]\:,   \label{15.9.3}
 \end{equation}%
 而反场$\phi _{n}^{\ddag }$的线性项提供了幂零条件:%
 \begin{equation}
 0=\Big( s\chi ^{m}\Big) \frac{\updelta_{L}(s\chi ^{n})}{\updelta \chi ^{m}}=s^{2}\chi ^{n}\:.   \label{15.9.4}
 \end{equation}%
 $\chi _{n}^{\ddag }$是外场, 在用$S[\chi ,\chi
 ^{\ddag }]$计算$S$-矩阵之前, 我们必须要赋予它合适的值. 由于这个原因, 我们引入一个鬼数为$-1$的任意费米泛函$\Psi [ \chi ]$, 
 并令\footnote{在这里没有必要区分左微分和右微分, 这是因为$\chi ^{n}$%
 ...
 S[\phi ,\updelta \Psi /\updelta \chi ]=I[\phi ]+(s\chi ^{n})\updelta \Psi \left[
 \chi \right] /\updelta \chi ^{n}=I[\phi ]+s\Psi [ \chi ]\:. 
 \label{15.9.6}
 \end{equation}
 ...
 A}\,\updelta f_{A}/\updelta \phi ^{r}$, $\omega _{C}^{\ddag }=0$, 以及$\omega _{A}^{\ast \ddag }=-f_{A}$.
 ...
 因为$\omega ^{\ast A}$和$h_{A}$有着{\kai{线性}}BRST变换, 所以影响其他$\chi ^{n}$的复杂性不会影响$\omega ^{\ast A}$和$h_{A}$(在这一点上, 
 ...
 因为$S_{\min }$的鬼数为零, 它对反场的级数展开必须采取如下的形式\begin{align}
 ...
 \begin{equation}
 0=f_{A}^{r}[\phi ]\,\frac{\updelta I[\phi ]}{\updelta \phi ^{r}}\:, 
 \label{15.9.9}
 \end{equation}%
 ...
 0 &=f_{A}^{r}[\phi ]\,\frac{\updelta f_{B}^{s}[\phi ]}{\updelta \phi ^{r}}%
 -f_{B}^{r}[\phi ]\,\frac{\updelta f_{A}^{s}[\phi ]}{\updelta \phi ^{r}}%
 +f^{C}{}_{AB}[\phi ]\,f_{C}^{s}[\phi ]  \nonumber \\
 &\quad+\frac{\updelta I[\phi ]}{\updelta \phi ^{r}}f^{rs}{}_{AB}[\phi ]\:, 
 \label{15.9.11}
 \end{align}%
 当场方程$\updelta I/\updelta \phi =0$成立时, 这变成了变换(\ref%
 {15.9.10})的对易关系(结构常数为$f^{C}{}_{AB}[\phi ]$). 主方程中反场的其他线性项左正比于$\omega ^{A}\omega ^{B}\omega ^{C}$并右正%
 比于$\omega _{D}^{\ddag }$, 它给出\begin{equation}
 0=f_{[A}^{r}[\phi ]\frac{\updelta f^{D}{}_{BC]}[\phi ]}{\updelta \phi ^{r}}%
 -f^{E}{}_{[AB}[\phi ]f^{D}{}_{C]E}[\phi ]+f^{rD}{}_{ABC}[\phi ]\frac{\updelta I[\phi ]}{\updelta \phi ^{r}}\:,   \label{15.9.12}
 \end{equation}%
 其中方括号代表对括号内的指标$A$, $B$和$C$做反对称化. 当场方程成立时, 它变成广义Jacobi恒等式(\ref{15.8.12}). 
 方程(\ref{15.9.11})对于对称性条件(\ref{15.9.9})(假定$f_{A}^{r}$构成规范对称性的完备基)的相容性是必须的,  
 而方程(\ref{15.9.12})对于对易关系(\ref{15.9.11})的相容性又是必须的. 注意到
 方程(\ref{15.9.11})和(\ref{15.9.12})中来自于$S_{\min }$中反场的二次项正比于$\updelta I[\phi ]/\updelta \chi $, 所以当场方程成立时, 它们为零, 
 并且在这一意义下, 这些是开对称性代数的特征. 主方程中反场的二阶项或更高阶项包含了$S_{\min }$中反场的三阶项和(或)更高%
 ...
 \begin{equation}
 (F,G)\equiv \frac{\updelta _{R}F}{\updelta \chi ^{n}}\frac{\updelta _{L}G}{\delta
 \chi _{n}^{\ddag }}-\frac{\updelta _{R}F}{\updelta \chi _{n}^{\ddag }}\frac{%
 \updelta _{L}G}{\updelta \chi ^{n}}\:.   \label{15.9.13}
 \end{equation}
 ...
 二项会改变符号\[
 \frac{\updelta _{R}S}{\updelta \chi _{n}^{\ddag }}\frac{\updelta _{L}S}{\updelta
 \chi ^{n}}=-\frac{\updelta _{L}S}{\updelta \chi _{n}^{\ddag }}\frac{\updelta _{R}S%
 }{\updelta \chi ^{n}}=-\frac{\updelta _{R}S}{\updelta \chi ^{n}}\frac{\updelta _{L}S}{%
 \updelta \chi _{n}^{\ddag }}\:. 
 \]%
 ...
 广义BRST变换定义为
 \begin{gather}
 \hat{\updelta}_{\theta }\chi ^{n}=\theta \frac{\updelta _{R}S}{\updelta \chi
 _{n}^{\ddag }}=-\theta (S,\chi ^{n})\:,   \label{15.9.16} \\
 \hat{\updelta}_{\theta }\chi _{n}^{\ddag }=-\theta \frac{\updelta _{R}S}{\updelta
 \chi ^{n}}=-\theta (S,\chi _{n}^{\ddag })\:,   \label{15.9.17}
 \end{gather}%
 其中$\theta $是无限小的费米参量. (当$S$的形式为(\ref{15.9.1})时, $\chi $的变换与原始BRST变换$\hat{\updelta}_{\theta }\chi ^{n}=\theta s\chi ^{n}$%
 相同.) 为了计算这一变换在一般泛函上的作用, 我们注意到反括号像导数一样作用, 也就是说
 \begin{equation}
 (F,GH)=(F,G)H\pm G(F,H)\:,   \label{15.9.18}
 \end{equation}%
 其中, 如果$G$是费米的而$F$是玻色的, 那么符号取$-1$, 否则就取$+1$. 
 如果$G$和$H$是$\chi$和$\chi ^{\ddag }$的任意泛函并满足$\hat{\updelta}_{\theta }G=-\theta (S,G)$和$\hat{%
 \updelta}_{\theta }H=-\theta (S,H)$, 那么
 \[
 \hat{\updelta}_{\theta }(GH)=-\theta (S,G)H-G\theta (S,H)=-\theta [(S,G)H\pm G(S,H)]\:, 
 \]%
 其中, 依据$G$是玻色的或费米的, 符号分别取$+$或$-$. 令方程(\ref{15.9.18})中的$F$等于玻色泛函$S$, 我们看到
 \[
 \hat{\updelta}_{\theta }(GH)=-\theta (S,GH)\:. 
 \]%
 再加上方程(\ref{15.9.16})和(\ref{15.9.17}), 这表明, 对于由场与反场的乘积之和构成的任意泛函$F$, 
 \begin{equation}
 \hat{\updelta}_{\theta }F=-\theta (S,F)\:.   \label{15.9.19}
 \end{equation}%
 主方程(\ref{15.9.14})可以解释为如下的表述, 即这些广义BRST变换保持$S$不变
 \begin{equation}
 \hat{\updelta}_{\theta }S=-\theta (S,S)=0\:.   \label{15.9.20}
 \end{equation}%
 ...
 否则为$+$, 对$F,$ $G$%
 和$H$的其他两个轮换取相应的符号. 取$F=G=S$, 方程(\ref{15.9.21})变成\[
 ...
 由于这一对称性, 主方程的解是不唯一的. 例如, 方程(\ref{15.9.22})表明对于任何给定的解$S$, 
 我们能够发现由如下的无限小变换给出的另一解
 \begin{equation}
 S^{\prime }=S+(\updelta F,S)\:,   \label{15.9.23}
 \end{equation}%
 其中$\updelta F$是$\chi $和$\chi ^{\ddag }$的无限小泛函, 为使$S^{\prime}$是玻色的且鬼数为零, 
 $\updelta F$必须是费米的且鬼数为$-1$, 除此以外, 它是任意的. 特别地, 
 取$\updelta F$仅为$\chi^{n}$的费米泛函$\epsilon \Psi $就给出
 \begin{equation}
 S^{\prime }[\chi ,\chi ^{\ddag }]=S[\chi ,\chi ^{\ddag }]+\epsilon \frac{%
 \updelta \Psi [ \chi ]}{\updelta \chi ^{n}}\frac{\updelta _{R}S}{\updelta \chi
 _{n}^{\ddag }}=S\left[ \chi ,\chi ^{\ddag }+\epsilon \frac{\updelta \Psi }{%
 \updelta \chi }\right] \:.   \label{15.9.24}
 \end{equation}%
 这些无限小变换可以被平庸地积掉, 这说明如果我们将反场平移至新的变量$\chi _{n}^{\ddag^{ \prime}
 }\equiv \chi _{n}^{\ddag }-\updelta \Psi /\updelta \chi ^{n}$, 主方程依旧是满足的.
 
 变换(\ref{15.9.23})是通常所说的正则变换的一个特殊情况, 在这里为了与第7章的正则变换相区分, 称其为``反正则%
 变换''. 反正则变换是场和反场的任意变换, 它可以是有限大或无限小的, 但它保如下的基本反括号关系不变:%
 \begin{equation}
 (\chi ^{n},\chi _{m}^{\ddag })=\updelta _{m}^{n} \:, \qquad 
 (\chi ^{n},\chi ^{m})=(\chi _{n}^{\ddag },\chi _{m}^{\ddag })=0\:. 
 \label{15.9.25}
 \end{equation}%
 例如, 考察由无限小费米算符$\updelta F$生成的无限小反正则变换, 在该变换下, 所有玻色泛函或费米泛函$G$被变换到
 \begin{equation}
 G\to G^{\prime }=G+(\updelta F,G)\:.   \label{15.9.26}
 \end{equation}%
 可以很容易证明这确实不改变基本反括号(\ref{15.9.25}). 为此, 注意到两个泛函$G$和$H$的反括号被变换到$(G^{\prime },H^{\prime })$, 保留到一阶无限小, 它是
 \[
 (G^{\prime },H^{\prime })=(G,H)+((\updelta F,G),H)+(G,(\updelta F,H))\:. 
 \]%
 利用Jacobi恒等式(\ref{15.9.21}), 这变为
 \begin{equation}
 (G^{\prime },H^{\prime })=(G,H)\pm (\updelta F,(G,H))\:,   \label{15.9.27}
 \end{equation}%
 ...
 \begin{equation}
 I_{\Psi }[\chi ]=S\left[ \chi ,\frac{\updelta \Psi [ \chi ]}{\updelta \chi}\right]   \label{15.9.28}
 \end{equation}%
 计算$S$-矩阵, 其中$\Psi [ \chi ]$%
 ...
 \begin{equation}
 \updelta _{\theta }\chi ^{n}=\theta s\chi _{n} \:, \qquad \text{其中} \quad 
 s\chi ^{n}=\left( \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]}{\delta
 \chi _{n}^{\ddag }}\right) _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi }\:,  \label{15.9.29}
 \end{equation}%
 在这样的变换下, 规范固定的作用量不变. 为了验证这点, 注意到
 \begin{align*}
 sI_{\Psi }[\chi ] &=\left( \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]}{\delta
 \chi _{n}^{\ddag }}\,\frac{\updelta _{L}S[\chi ,\chi ^{\ddag }]}{\updelta \chi ^{n}%
 }\right) _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi } \\
 &\quad+\left( \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]}{\updelta \chi _{m}^{\ddag }%
 }\,\frac{\updelta _{L}^{2}\Psi [ \chi ]}{\updelta \chi ^{m}\updelta \chi ^{n}}%
 \,\frac{\updelta _{L}S[\chi ,\chi ^{\ddag }]}{\updelta \chi _{n}^{\ddag }}\right)
 _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi }\:. 
 \end{align*}%
 右边的第一项根据主方程为零, 而第二项由于被求和的函数$m$和$n$是反对称\footnote{若$\chi ^{n}$和$\chi ^{m}$都是玻色的,
 这是因为$\updelta S/\updelta \chi ^{n}$和$\delta S/\updelta \chi ^{m}$反对易. 若$\chi ^{n}$和$%
 \chi ^{m}$中一个是玻色的而另一个是费米的, 这是因为$S$对费米的$\chi $的右导数和左导数符号相反. 若$\chi ^{n}$和$\chi ^{m}$都是费%
 米的, 由于$ \updelta _{L}^{2}\Psi /\updelta \chi ^{m}\updelta \chi ^{n}$是反对称的, 所以该项是反对称的.}的, 所以也为零.
 
 对于闭代数且形如(\ref{15.9.1})的$S$, 变换(\ref{15.9.29})是原始的BRST变换$\updelta _{\theta }\chi ^{n}=\theta s\chi ^{n}$.
 但对于一般的开代数, 变换(\ref{15.9.29})与原始BRST变换并不相同, 并且, 一般而言, 除非场方程被满足,
 它甚至不是幂零的. 反而, 从主方程中偏移场$%
 \chi _{n}^{\ddag ^{\prime }}=\chi _{n}^{\ddag }-\updelta \Psi [ \chi
 ]/\updelta \chi ^{n}$的一阶项中, 我们发现
 \begin{equation}
 s^{2}\chi ^{m}=\mp \left( \frac{\updelta _{L}}{\updelta \chi _{m}^{\ddag }}\frac{%
 \updelta _{R}}{\updelta \chi _{n}^{\ddag }}I[\chi ,\chi ^{\ddag }]\right) _{\chi
 ^{\ddag }=\updelta \Psi /\updelta \chi }\frac{\updelta I_{\Psi }[\chi ]}{\updelta
 \chi ^{n}}\:,   \label{15.9.30}
 \end{equation}%
 其中, 根据$\chi ^{m}$是玻色的还是费米的, 符号分别是$-$和$+$. 我们再次看到了开规范代数的这种依赖场方程的特征与$S$中反场的二次项相联系.
 ...
 \chi _{n}^{\ddag }=\updelta \Psi [ \chi ]/\updelta \chi ^{n}$从$%
 S[\chi ,\chi ^{\ddag }]$中得到的, 换句话说, 是通过令$\chi _{n}^{\ddag ^{\prime
 }}=0$得到的. 我们希望计算$\Psi [ \chi ]$的变化对这些矩阵元的影响. 首先考察真空-真空振幅
 \begin{equation}
 Z_{\Psi }=\int \left[ \prod \dif\chi \right]\: \exp (\mi I_{\Psi }[\chi ]) \:.   \label{15.9.31}
 \end{equation}%
 对$\Psi [ \chi ]$偏移$\updelta \Psi [ \chi ]$, 它的变化是
 \begin{equation}
 \updelta Z=\mi\int \left[ \prod \dif\chi \right]\: \exp (\mi I_{\Psi }[\chi
 ])\left( \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]}{\updelta \chi _{n}^{\ddag }}%
 \right) _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi }\left( \frac{\updelta
 (\updelta \Psi [ \chi ])}{\updelta \chi ^{n}}\right) \:. \label{15.9.32}
 \end{equation}%
 在场空间中分部积分, 于是
 \begin{align}
 \updelta Z &=\int \left[ \prod \dif\chi \right] \:\exp (\mi I_{\Psi }[\chi ])
 \nonumber \\
 &\quad\times \left\{ \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]}{\updelta \chi
 _{n}^{\ddag }}\frac{\updelta _{L}I_{\Psi }[\chi ]}{\updelta \chi ^{n}}-\mi\Delta
 S[\chi ,\chi ^{\ddag }]\right\} _{\chi ^{\ddag }=\updelta \Psi /\updelta \chi
 }\updelta \Psi [ \chi ]\:,   \label{15.9.33}
 \end{align}%
 其中\begin{equation}
 \Delta \equiv \frac{\updelta _{R}}{\updelta \chi _{n}^{\ddag }}\frac{\updelta _{L}%
 }{\updelta \chi ^{n}}\:.   \label{15.9.34}
 \end{equation}
 我们看到, 一般而言, 真空-真空振幅不依赖$\Psi$的条件{\kai{不}}是主方程(\ref{15.9.2}), 而是所谓的{\kai{量子主方程}}
 \begin{equation}
 \text{在 } \:\chi^{\ddag}=\updelta \Psi/\delta\chi  \text{ 时,}\qquad \quad  (S,S) - 2 \mi \Delta S = 0 \:.  \label{15.9.35}
 \end{equation}
 ...
 $S$中$\hbar$的零阶项都满足原始主方程(\ref{15.9.2}). 通常情况下, 构造一个满足经典主方程的作用量很容易, 这会使方程(\ref{15.9.35})中的第一项为零, 
 这样一来, 问题就变成第二项是否也为零. 在第22章关于反常的讨论中, 我们会考察定域作用量不满足量子主方程的情况.
 
 假定量子主方程(\ref{15.9.35})是满足的, 算符$\mathcal{O}[\chi ]$的真空期望值因$\Psi $变化$\updelta \Psi $而引起的变化是
 \begin{equation}
 \updelta \langle \mathcal{O}\rangle =\frac{-\mi}{Z_{\Psi }}\int \left[ \prod %
 \dif\chi \right] \exp \left( \mi I_{\Psi }[\chi ]\right) \frac{\updelta _{R}%
 \mathcal{O}[\chi]}{\updelta \chi ^{n}}\left( \frac{\updelta _{R}S[\chi ,\chi ^{\ddag }]%
 }{\updelta \chi _{n}^{\ddag }}\right) _{\chi _{n}^{\ddag }=\updelta \Psi /\updelta
 \chi }\updelta \Psi [ \chi ]\:.   \label{15.9.36}
 \end{equation}%
 在方程(\ref{15.9.36})的被积函数中, 指数的系数正是$s\mathcal{O[\chi ]}$. 我们看到, 对于在广义BRST变换(\ref{15.9.16})---(\ref{15.9.17})%
 下不变的算符,\footnote{对于开规范理论, 一般而言, 除了常数以外, 没有在变换(\ref{15.9.16})---(\ref{15.9.17})下不变的算符.  取而代之, 应该考虑的是在幂零的``量子BRST算符'' $\sigma$下不变的算符$O(\chi,\chi^{\ddag})$, 其中$\sigma$定义为$\sigma O = (O,S)-\mi \Delta O$. 对于仅依赖于$\chi$的$O$, 这一条件退化为方程(\ref{15.9.36}). 如果$\sigma O=0$, 那么规范固定费米子$\Psi$的微小变换不会影响$O(\Psi, \updelta\Psi/\updelta \chi)$的期望值.\cite{24}} 它们的真空期望值不被规范固定费米子$\Psi $的变化所影响. 对于两个或多个算符的真空期望值, 相应结果同样成立.
 
 
 \section*{附录~A\quad 关于Lie代数的一个定理}

\addcontentsline{toc}{section}{附录~A\quad 关于Lie代数的一个定理}                %自动提目录
\markright{附录~A\quad 关于Lie代数的一个定理}      %%前双后单书眉
...
在这个附录中, 我们考察生成元为$t_{\alpha }$且结构常数为$C^{\alpha }{}_{\beta \gamma }$的一般Lie代数$\mathscr{G}$, 并将证%
明如下3个条件的等价性:
...
\noindent {\boldmath \bf{c}:} Lie代数$\mathscr{G}$是互相交换的紧致单子代数与$U(1)$子代数的直和, 记为子代数$\mathscr{H}_{m}$的直和.
...
\mathscr{H}_{m}$, $a$标记该子代数下的各个生成元, 使满足方程(\ref{15.A.1})的矩阵$g_{ma,nb}$采取如下形式
\begin{equation}
g_{ma,nb}=g_{m}^{-2}\updelta _{mn}\updelta _{ab}\:,   \label{15.A.2} \end{equation}%
其中$g_{m}^{-2}$是任意的正实数.
...
}]_{ab}\,\updelta _{mn}\:,   \label{15.A.6}
\end{equation}%
...
(t^{\mathrm{A}(m)}{}_{\ell c})_{ab}\,\updelta _{mn}\:.   \label{15.A.7}
\end{equation}%
但是, 既然它是全反对称的, 并正比于$\updelta _{mn}$, 它必须也正比于$\updelta _{\ell n}$和$\updelta _{\ell m}$:%
\begin{equation}
\tilde{C}_{\ell c,ma,nb}=\updelta  _{\ell n}\,\updelta  _{\ell m}\,C_{cab}^{(\ell )}\:.   \label{15.A.8}
\end{equation}%
换句话说, 在这组基下, 对Lie代数的{\kai{任何}}表示$t^{(m)}{}_{a}\equiv t_{ma}$, 我们有
\begin{equation}
[ t^{(m)}{}_{a},t^{(m)}{}_{b}]=\mi\,\updelta_{mn}C_{cab}^{(m)}t^{(m)}{}_{c}\:,   \label{15.A.9}
\end{equation}%
其中$C_{cab}^{(m)}$是实的且关于指标$a$, $b$, $c$全反对称. 当我们说Lie代数是子代数$%
t^{(m)}$的{\kai{直和}}时, 我们是指, 能够找到一组基, 在这组基下, 生成元落入集合$t^{(m)}$中, 并且一个集合内的生成元, 它%
们彼此之间的对易子由该集合内的生成元的线性组合给出, 并且一个集合内的所有生成元与所有其他集合内的所有生成元对易.
...
\[
[ t_{a}^{(m)},t_{b}^{(m)}]=\mi\updelta  _{nm}C^{(m)c}{}_{ab}t_{c}^{(m)} \:, 
\]
...
\begin{equation}
g_{ma,nb}\equiv g_{ab}^{(m)}\updelta  _{mn}\:,   \label{15.A.11}
\end{equation}%
...
\section*{附录~B\quad Cartan目录}

\addcontentsline{toc}{section}{附录~B\quad Cartan目录}                %自动提目录
\markright{附录~B\quad Cartan目录}      %%前双后单书眉
...
\end{KAI}    
 ...
 \begin{thebibliography}{99}
    ...
    \bibitem {7}{\bf{a}} (其中$g_{\alpha \beta}=\updelta _{\alpha \beta}$)蕴含{\bf{b}}, {\bf{b}}蕴含{\bf{a}}的证明是M. Gell-Mann和S. L. Glashow给出的, {\textit{Ann. Phys. (N.Y.)}} {\bf{15}}, 437 (1961).
    ...
    \bibitem {18}反-BRST变换被L. Alvarez-Gaum\`{e}和L. Baulieu推广至一般的定域系统, {\textit{Nucl. Phys.}} {\bf{B212}}, 255 (1983). 它由如下算符诱导:
        \begin{align*}
        \bar{s} &= \omega^{\ast A} \updelta _{A} \phi^{r} \frac{\updelta _{L}}{\updelta  \phi^{r}} - \frac{1}{2}\omega^{\ast B} \omega^{\ast C} f^{A}{}_{BC} \frac{\updelta _{L}}{\updelta  \omega^{\ast A}} - \omega^{\ast B} h^{C} f^{A}{}_{BC} \frac{\updelta }{\updelta  h^{A}}\\
        &\quad+\Big[ - f^{A}{}_{BC}\omega^{\ast B}\omega^{C} + h^{A} \Big] \frac{\updelta _{L}}{\updelta \omega^{A}} \:. 
        \end{align*}
        可以直接验证这一变换是幂零的:
        \[
        \bar{s}^{2}=0 \:. 
        ...
        \]
        (用Hodge算符$\bar{t}\equiv \omega^{A}\updelta /\updelta  h^{A}$取代$t$即可.) 反-BRST对称有助于枚举拉格朗日量和Green函数中可能的项, 但我不知道它在什么情况下是必不可是的.
