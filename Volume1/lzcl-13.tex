...
\chapter{红外效应} \label{cha:13}
 \thispagestyle{empty} \marginpar[\flushright{\raisebox{17ex}[0pt]{{\small[534]\hspace*{5mm}}}}]{{\raisebox{17ex}[0pt]{\small\hspace*{5mm}[534]}}}
  \markboth{第13章\quad 红外效应}{第13章\quad 红外效应}
...
那么我们就要\marginpar[\flushright{\small[535]\hspace*{5mm}}]{{\small\hspace*{5mm}[535]}}给$\alpha\to\beta$的$S$-矩阵元乘上一个额外的带电粒子传播子, 该传播子携带着带电粒子在发射光子之前所拥有的动量$p+q$, 再乘上新的带电粒子\lzx 光子顶点给出的贡献. 对于零自旋, 质量为$m$而电荷为$+e$的带电粒子, 这些因子是
\[
...
^{\prime })=-\mi\delta _{\sigma ,\sigma ^{\prime }}\,p^{\mu }/p^{0}
\]%
...
没有因子在$q\to 0$时趋于$(p\cdot q)^{-1}$. 因此, 在过程$\alpha\to\beta$中发射\,4\,-动量为$q$且极化指标为$\mu$的单个软光子的振幅$M_{\beta \alpha}^{\mu }(q)$(不带有能动量守恒$\delta$-函数的$S$-矩阵), %
...
这给出光子发射矩阵元(省略掉$\delta$-函数的$S$-矩阵元)%
\begin{align}
...
为了计算发射$N$个具有确定能量$\omega _{r}\equiv |\bq_{r}|$的软光子的微分速率, 我们必须要把方程(\ref{13.3.3})中光子动量$\bq_{r}$ 的方向积掉. 这些积分与我们在积分(\ref{13.2.8})中遇到的是一样,%
\begin{equation}
...
}{\beta _{nm}\sqrt{1-\beta _{nm}^{2}}}\ln \left( \frac{1+\beta _{nm}}{1-\beta _{nm}}\right)\:.  \label{13.3.12}
\end{equation}
...
\begin{equation}
S_{ba}=\delta (b-a)-2\mi\uppi \delta (E_{a}-E_{b})T_{ba}\:,  \label{13.4.1}
\end{equation}%
其中\begin{equation}
...
如果方程(\ref{13.4.2})中的第一个中间态$c_{1}$由初态$\alpha$中的粒子构成, %
...
算符$\Omega _{b}^{-}$和$\Omega _{a}^{+}$分别本将是将自由粒子态转化到``出''态和``入''态的幺正算符. 因此, %
...
我们可以假定真实的无质量粒子总是作为喷注伴随着软量子全体产生, 这里的全体软量子在某些动量空间体积内是相同的. %
...
不过现在计算所用的\,Hilbert\,空间是在仅有软光子构成的约化\,Hilbert\,空间, %
...
};p^{\prime },\sigma ^{\prime })&=\mi(2\uppi )^{4}\delta ^{4}(q+p-q^{\prime
}-p^{\prime })  \nonumber \\
...
我们现在重复一下在第10章给出的并在\,\ref{sec:13.1}\,节使用过的标准极点学讨论. %
...
$e\delta_{\sigma^{\prime},\sigma }$, 其中$e$是粒子电荷. 这样, 流守恒条件就告诉我们, 到动量的第二阶,%
\[
(\bp^{\prime }-\bp)\cdot \bG_{\sigma ^{\prime },\sigma
}(\bp^{\prime },\bp)=\left( \frac{\bp^{\prime 2}}{2m}-%
\frac{\bp^{2}}{2m}\right) e\delta _{\sigma ^{\prime },\sigma }\:.%
\]%
因此, $\bG$中动量的一阶项是$e(\bp^{\prime }+\bp)\delta_{\sigma^{\prime},\sigma}/2m$加上可能的与%
...
从而
\begin{equation}
N^{ik}=-\frac{e^{2}}{m}\delta _{ik}+\text{线性} \:. \label{13.5.13}
\end{equation}%
由于$G^{i}$至少是小动量的一阶项, 那么方程(\ref{13.5.3})中$M^{ik}$的极点项也将是这样, %
于是在零阶仅给我们留下了{\KAI{非极点}}项$N^{ik}$%
\begin{equation}
M^{ik}(0;0,0)=N^{ik}(0;0,0)=-\frac{e^{2}}{m}\delta _{ik}\:. \label{13.5.14}
\end{equation}
...
\section[外场近似]{外场近似{}$^*$\footnote{$^*${}本节或多或少的在本书发展的主线之外, 可以在第一次阅读时略过.}} \label{sec:13.6}
\setcounter{equation}{0}

重的带电粒子, 像原子核, 在作用上近似地像一个经典外场的源, 这在直觉上是显然的. 本节,
我们将看到如何证明这一近似是合理的, 并得到其局限性的一些概念.

考察这样的\,Feynman\,图或\,Feynman\,图的一部分, 其中重带电粒子从初态到末态穿过图并发射$N$个\,4\,-动量为%
...
\end{align}%
矩阵元中包含了重粒子可以参与的所有相互作用, 包括强核力. 该振幅在$q_{1},q_{2},\cdots q_{N}\to  0$时有一个高阶极点, 这个极点来自流乘积矩阵元中中间态仅由初态和末态中的重粒子构成的那些项. 当$q_{1},q_{2},\cdots q_{N}$的所有分量与(可能是复合粒子的)重粒子动力学相关的所有能量和动量相比很小时, 这一高阶极点将主导(\ref{13.6.1})\marginpar[\flushright{\small[557]\hspace*{5mm}}]{{\small\hspace*{5mm}[557]}}.
...
&\quad\times \delta ^{4}(p^{\prime }+q_{1}+q_{2}+\cdots +q_{N}-p)\sum_{\sigma
...
首先注意到, 在粒子静止的\,Lorentz\,系中, 旋转不变性要求流矩阵元的空间分量为零而时间分量正比于$\delta _{\sigma ^{\prime },\sigma }$, 且除此之外不再依赖$\sigma$或$\sigma ^{\prime}$. 比例常数由方程(\ref{10.6.3})给出, %
然后做一个\,Lorentz\,变换就给出了方程(\ref{13.6.4}).}%
\begin{equation}
\langle p,\sigma ^{\prime }|J^{\mu }(0)|p,\sigma \rangle =\frac{Z\,e\,p^{\mu
}\delta _{\sigma ^{\prime }\sigma }}{p^{0}(2\uppi )^{3}}\:,
\label{13.6.4}
\end{equation}%
从而使\begin{equation}
\mathscr{G}_{\sigma ^{\prime }\sigma }^{\mu }(p)=2Z\,e\,p^{\mu }\delta _{\sigma
^{\prime }\sigma }\:.  \label{13.6.5}
\end{equation}%
方程(\ref{13.6.5})关键之处在于指出这些矩阵都是对易的, 所以它们的乘积可以从对置换的求和中分解出来:\marginpar[\flushright
{\raisebox{-26ex}[0pt]{{\small[558]\hspace*{5mm}}}}]{{\raisebox{-26ex}[0pt]{\small\hspace*{5mm}[558]}}}
\begin{align}
...
p^{0}(2\uppi )^{3}}(2\uppi )^{4}\delta ^{4}(p^{\prime }+q_{1}+q_{2}+\cdots
+q_{N}-p)\delta _{\sigma ^{\prime },\sigma }  \nonumber \\
...
\end{align}%
到$q$的领头阶, 这里的$\delta$-函数可以写成
\begin{equation}
\delta ^{4}(p^{\prime }+q_{1}+\cdots +q_{N}-p)=p^{0}\delta ^{3}(\bp%
^{\prime }+\bq_{1}+\cdots +\bq_{N}-\bp)\delta (p\cdot
...
&+\text{置换}\Biggr]=(2\mi\uppi )^{N-1}\delta (p\cdot q_{1})\delta
(p\cdot q_{2})\cdots \delta (p\cdot q_{N-1})\:.  \label{13.6.8}
\end{align}%
例如, 对$N=2$, 这给出:%
\[
\frac{1}{[p\cdot q_{1}-\mi\epsilon ]}+\frac{1}{[p\cdot q_{2}-\mi\epsilon ]}=%
\frac{1}{[p\cdot q_{1}-\mi\epsilon ]}+\frac{1}{[-p\cdot q_{1}-\mi\epsilon ]}%
=2\mi\uppi \delta (p\cdot q_{1})\:.
\]%
...
\begin{align}
&\mathscr{G}_{\sigma ^{\prime },\sigma }^{\mu _{1}\mu _{2}\cdots \mu
_{N}}(q_{1},q_{2},\cdots q_{N};p) \to  (Ze)^{N}(2\uppi )^{N}\delta
_{\sigma ^{\prime },\sigma }p^{\mu _{1}}p^{\mu _{2}}\cdots p^{\mu _{N}} \nonumber \\
&\times \delta ^{3}(\bp^{\prime }+\bq_{1}+\bq%
_{2}+\cdots +\bq_{N}-\bp)\delta (p\cdot q_{1})\delta (p\cdot
q_{2})\cdots \delta (p\cdot q_{N})\:.  \label{13.6.9}
\end{align}%
...
&\times \delta ^{3}(\bp^{\prime }+\bq_{1}+\bq%
_{2}+\cdots +\bq_{N}-\bp)\delta (q_{1}^{0})\delta
(q_{2}^{0})\cdots \delta (q_{N}^{0})\delta _{\sigma ^{\prime },\sigma }\:,  \label{13.6.10}
\end{align}%
...
假定这个粒子既出现在初态又出现在末态. 对方程(\ref{13.6.10})中的$\delta$-函数使用\,Fourier\,表示, %
...
\pi Z\,e\:n^{\mu _{r}}\,\delta (q_{r}^{0})\me^{-\mi\bq_{r}\cdot \bX}
\label{13.6.11}
\end{align}%
其中$\psi (\bX)$是坐标空间波函数:%
\begin{equation}
...
\Bigl[ 2\uppi Z\,e\:n_{\mu }\delta (q^{0})\me^{-\mi\bq\cdot \bX}\Bigr] %
\Bigl[ (2\uppi )^{4}\,e\,\gamma ^{\mu }\,\delta ^{4}(k-k^{\prime }-q)\Bigr]
\label{13.6.13}
\end{equation}%
...
\frac{2\uppi \,Z\,e\:n^{\mu }\delta (q^{0})\me^{-\mi\bq\cdot \bX}}{q^{2}-\mi\epsilon }\right] \:.  \label{13.6.15}
\end{equation}%
...
这会使能量分母很小, 而图13.5中的其他所有图所对应的中间态, 要么有额外的光子, 要么有额外的电子\lzx 正电子对, 要么有重的粒子\lzx 反粒子对. 它们被大能量分母压低了.) 非交叉梯形图的求和可以通过解一个积分方程求出, %
这一积分方程称为\,\emph{Bethe-Salpeter}{\KAI{(贝特\lzx 萨尔皮特)方程}},\textsuperscript{\cite{10}} 然而, 除非两个粒子都是非相对论性的, 否则没有什么合适的理由只挑出图的这个子集, 在两个粒子均为非相对论性的情况下, Bethe-Salpeter\,方程就退化成了%
...
这里这些软标量粒子的总能量小于某个小量$E_{T}$. 计入能量小于某个小量$\Lambda$的软标量粒子引起的辐射修正.

3. 对低能光子在任意靶上的散射, 推导方程(\ref{13.5.14})的下一项贡献的公式.
