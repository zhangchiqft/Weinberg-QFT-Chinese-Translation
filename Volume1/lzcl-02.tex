...
本书的观点是量子场论是(在一定条件下)协调量子力学和狭义相对论的唯一途径.
因此我们的第一个任务就是研究类似\,Lorentz\,不变性这样的对称性怎样出现在量子中.

\section{量子力学} \label{sec:2.1}
...
特别地, 一个无限接近平庸变换\marginpar[\flushright{\raisebox{-3ex}[0pt]{{\small[52]\hspace*{5mm}}}}]{{\raisebox{-3ex}[0pt]{\small\hspace*{5mm}[52]}}}的对称变换可以用一个无限接近单位算符的线性幺正算符表示:
\begin{equation}
...
\end{equation}
正号和负号分别对应于$U(T_{2}T_{1})$是\marginpar[\flushright{\raisebox{-3ex}[0pt]{{\small[53]\hspace*{5mm}}}}]{{\raisebox{-3ex}[0pt]{\small\hspace*{5mm}[53]}}}幺正算符和反幺正算符的情况. 由于$\Psi_{A}$和$\Psi_{B}$线性独立, 因而只有一种可能
\begin{equation}
...
\end{equation}
如前所述, 在物理\,Hilbert\,空间上, 这类连续群的变换只能由幺正(而不是反幺正)算符$U(T(\theta))$表示. 对于一个\,Lie\,群, 至少在其单位元的有限邻域内, 这些算符可以用幂级数表示\marginpar[\flushright{\raisebox{-3ex}[0pt]{{\small[54]\hspace*{5mm}}}}]{{\raisebox{-3ex}[0pt]{\small\hspace*{5mm}[54]}}}
\begin{equation}
...
\end{equation}
由于某些目的, 将\,Lorentz\,变换写成另一种的形式是有用的.
...
\end{equation}
(注意到, 如果$\Lambda^{\mu}{}_{\!\nu}$和$\bar{\Lambda}^{\mu}{}_{\!\nu}$均满足方程(\ref{2.3.5}), 那么$\bar{\Lambda}^{\mu}{}_{\!\rho}\Lambda^{\rho}{}_{\!\nu}$也满足这个方程, 所以这是一个\,Lorentz变换. 这里用到上横线只是为了区分两个\,Lorentz\,变换.) 因\marginpar[\flushright{{\small[57]\hspace*{5mm}}}]{{\small\hspace*{5mm}[57]}}此作用在物理态上的变换$T(\Lambda,a)$满足合成规则
\begin{equation}
...
而这些算符满足合成规则\marginpar[\flushright{\raisebox{-4ex}[0pt]{{\small[82]\hspace*{5mm}}}}]{{\raisebox{-4ex}[0pt]{\small\hspace*{5mm}[82]}}}
\begin{equation}
...
\subsection*{$\left(  A\right)  $ \quad 代数}

有中心荷的情况下, 代替方程(\ref{2.4.12})\yzx (\ref{2.4.14}), 非齐次\,Lorentz\,群生成元的对易关系是\vspace{-.1mm}
\begin{align}
\mi[J^{\mu\nu},J^{\rho\sigma}] &= \eta^{\nu\rho}J^{\mu\sigma}
-\eta^{\mu\rho}J^{\nu\sigma}-\eta^{\sigma\mu}J^{\rho\nu}\nonumber\\
...
\subsection*{$\left(  B\right)  $ \quad 拓扑}
...
\right)
\]
产生的\,Lorentz\,变换$\Lambda(\lambda(\theta))$是一个绕第\,3\,轴的旋转, 旋转角度是$\theta$, 因此$\lambda=-1$产生的是角度为$2\uppi$的旋转. Lorentz 群与$SL(2,C)$不相同, 但\marginpar[\flushright{\small[88]\hspace*{5mm}}]{{\small\hspace*{5mm}[88]}}与
...
总存在另外一个群, 它的结果除了没有超选择定则以外与原先的结果是相同的.}}
...
\[
\Upsilon_{k}^{\prime}=\sum\nolimits_{l}c_{kl}\Psi_{l}^{\prime} \:.
\]
从方程(\ref{2.A.1})中我们得到
\[
...
\end{align*}
再\marginpar[\flushright{\small[96]\hspace*{5mm}}]{{\small\hspace*{5mm}[96]}}次利用方程(\ref{2.A.14}), 给出
\begin{equation}
...
$\Psi_{\bp,\sigma,n}^{\prime}=\sum_{m}\mathscr{U}_{mn}\Psi_{\bp,\sigma,m}$, 对这个态, 我们会得到相同的变换
...
\end{equation}
其中$\mathscr{A}$是对称且幺正的, 而$\mathscr{B}$的对角元全为零. 因为$\mathscr{A}$是对称的,  它可以表示为一个对称反厄米矩阵的指数, 所以, 通过仅作用在$\mathscr{A}$上的变换(\ref{2.C.2})就\marginpar[\flushright{\small[102]\hspace*{5mm}}]{{\small\hspace*{5mm}[102]}}可以使其对角化, $\mathscr{U}$的相应子矩阵是实的并且因此是正交的. 因此只需考虑联系$\me^{\mi\phi_{n}}\neq1$ 的行与列的子矩阵$\mathscr{B}$. 对于$n\neq m$, 方程(\ref{2.C.4})给出$\mathscr{T}_{nm}=\me^{\mi\phi_{n}}\mathscr{T}_{mn}$ 和%
...
\bibitem{15} E. P. Wigner, in {\textit{Group Theoretical Concepts and Methods in Elementary Particle Physics,}} F. G\"{u}rsey\,编辑\,(Gordon and Breach, New York, 1964): p. 37.
\end{thebibliography}
